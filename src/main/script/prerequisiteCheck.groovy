...
def checkDotnet() {
    ...
    if (matcher.size() > 0) {
        ...
        // `dotnet --version` reports the SDK version, not the TargetFramework: the
        // csproj files target "net8.0" (LTS), which needs the .NET 8.0 SDK or newer.
        def result = checkVersionAtLeast(curVersion, "8.0.0")
        if (!result) {
