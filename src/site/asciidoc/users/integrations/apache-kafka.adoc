...
They can be built from source from the latest 0.8 https://plc4x.apache.org/users/download.html[release] of
...
Producing to the kafka topic using the sample packet shown below should result in the value 655 being sent to the
tag 'running' PLC field which is mapped to the address 'RANDOM/Temporary:Integer'.
...
The field value holds an alias for the PLC address. The field-alias mapping is handled in the connector configuration.

'value' holds the value that is being sent to the field.

'expires' is used to discard a record if it has taken too long to be sent to the PLC. It is a duration in milliseconds.
It will be dicarded upon being recieved by the sink task if the record timestamp + 'expires' > current time.
...
This is the second release of the source Kafka connector and the first release of the sink connector. Between the
previous release and this one no Schema changes have occurred.

Going forward we expect that the Schema compatibility will remain backward compatible.