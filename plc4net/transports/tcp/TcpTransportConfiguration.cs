...
{
    /// <summary>
    /// Settings for the TCP transport. The defaults are this port's own choices and
    /// do not all match the Java SPI3 transport, which defaults keep-alive to false
    /// and the send buffer size to 81920.
    /// </summary>
    public class TcpTransportConfiguration : ITransportConfiguration
    {
        ...
        /// <summary>
        /// Time in milliseconds allowed for resolving the host name and, separately, for
        /// establishing the TCP connection. Zero or less means no timeout of our own; the
        /// operating system's limit still applies.
        /// </summary>
        public int ConnectTimeout { get; set; } = 5000;

        /// <summary><see cref="ConnectTimeout"/> in the form a <c>CancellationTokenSource</c> takes.</summary>
        internal int ConnectTimeoutMillis => ConnectTimeout > 0 ? ConnectTimeout : Timeout.Infinite;
