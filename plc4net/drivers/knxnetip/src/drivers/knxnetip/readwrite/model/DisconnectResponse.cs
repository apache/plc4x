...
{
    public partial class DisconnectResponse : KnxNetIpMessage
    {
        public override ushort MsgType => (ushort) (0x020A);
        ...
        public static new DisconnectResponse StaticParse(ReadBuffer readBuffer)
        {
            var communicationChannelId = readBuffer.ReadByte("communicationChannelId", 8);
            var status = (Status) readBuffer.ReadByte("status", 8);
            return new DisconnectResponse(communicationChannelId, status);
        }
