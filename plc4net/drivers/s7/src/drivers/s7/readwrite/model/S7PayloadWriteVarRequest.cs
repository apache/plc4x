...
{
    public partial class S7PayloadWriteVarRequest : S7Payload
    {
        public override byte MessageType => (byte) (0x01);

        public System.Collections.Generic.List<S7VarPayloadDataItem> Items { get; }

        public S7PayloadWriteVarRequest(System.Collections.Generic.List<S7VarPayloadDataItem> items)
        {
            Items = items;
        }

        public static new S7PayloadWriteVarRequest StaticParse(ReadBuffer readBuffer, byte messageType, S7Parameter parameter)
        {
            var items = new System.Collections.Generic.List<S7VarPayloadDataItem>();
            var _itemsCnt = (int) (((S7ParameterWriteVarRequest) parameter).Items.Count);
