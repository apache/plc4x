//
// Licensed to the Apache Software Foundation (ASF) under one
// or more contributor license agreements.  See the NOTICE file
// distributed with this work for additional information
// regarding copyright ownership.  The ASF licenses this file
// to you under the Apache License, Version 2.0 (the
// "License"); you may not use this file except in compliance
// with the License.  You may obtain a copy of the License at
//
//      https://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing,
// software distributed under the License is distributed on an
// "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
// KIND, either express or implied.  See the License for the
// specific language governing permissions and limitations
// under the License.
//

// Code generated from the mspec by plc4net-code-gen. DO NOT EDIT.

using System;
using System.Linq;
using org.apache.plc4net.spi.drivers;
using org.apache.plc4net.spi.generation;

namespace org.apache.plc4net.drivers.s7.readwrite.model
{
    public partial class TPKTPacket : IMessage
    {
        public const byte ProtocolId = 0x03;

        public COTPPacket Payload { get; }

        public TPKTPacket(COTPPacket payload)
        {
            Payload = payload;
        }

        public static TPKTPacket StaticParse(ReadBuffer readBuffer)
        {
            var protocolId = readBuffer.ReadByte("protocolId", 8);
            if (!Equals(protocolId, (byte) (0x03)))
                throw new ParseException($"Expected constant {ProtocolId} for 'protocolId' but got {protocolId}");
            {
                var reserved = readBuffer.ReadByte("reserved", 8);
                if (!Equals(reserved, (byte) (0x00))) { /* mspec reserved: value differs from the spec default */ }
            }
            var len = readBuffer.ReadUshort("len", 16);
            var payload = COTPPacket.StaticParse(readBuffer, (ushort) ((len - 4)));
            return new TPKTPacket(payload);
        }

        public void Serialize(WriteBuffer writeBuffer)
        {
            writeBuffer.WriteByte("protocolId", 8, ProtocolId);
            writeBuffer.WriteByte("reserved", 8, (byte) (0x00));
            writeBuffer.WriteUshort("len", 16, (ushort) ((Payload.GetLengthInBytes() + 4)));
            Payload.Serialize(writeBuffer);
        }

        public int GetLengthInBits()
        {
            var lengthInBits = 0;
            lengthInBits += 8;
            lengthInBits += 8;
            lengthInBits += 16;
            lengthInBits += Payload.GetLengthInBits();
            return lengthInBits;
        }

        public int GetLengthInBytes() => GetLengthInBits() / 8;
    }
}
