...
RTU), Siemens S7 (ISO-on-TCP / COTP) and KNXnet/IP (UDP tunnelling).
...
├── test/                                             (test projects)
│   ├── spi-test/               plc4net-spi-test      (404 xUnit tests)
...
│   └── knxnetip/               plc4net-driver-knxnetip   (src/.../readwrite/model is generated)
├── transports/
│   ├── tcp/                    plc4net-transports-tcp
│   ├── udp/                    plc4net-transports-udp    (datagram; KNXnet/IP)
...
└── docs/
    ...
    ├── hardware-verification.md                        (physical-device runs)
    ├── packaging.md                                    (NuGet pack/consume)
    └── images/                                         (PLC-side evidence)
```
...
Generated code (`knxnetip` 180 model files, `code-gen` ANTLR output) keeps its
...
| C-19 | KNXnet/IP driver - UDP tunnelling: SEARCH / CONNECT / CONNECTIONSTATE heartbeat / DISCONNECT handshake, group-value Read (correlates the `GroupValueResponse` off the bus) and Write, a bus-monitor callback, `:DPT…`-driven datapoint decoding over the generated `KnxDatapoint` codec | drivers/knxnetip |
| C-20 | Modbus, S7 and KNXnet/IP wire models, generated and round-tripped: Modbus TCP + S7 against the shared ParserSerializerTestsuite.xml (6 + 11 vectors), the S7 / Modbus `DataItem` `dataIo` against hand-built IEC-61131 vectors, and the KNXnet/IP handshake + tunnelling frames against a scripted gateway on a loopback socket | drivers/{modbus,s7,knxnetip}/src/.../readwrite/model |
| C-21 | UDP datagram transport (bound socket, connected to one peer, background receive loop into a ring buffer, local endpoint exposed for the KNX HPAI) | transports/udp |
| C-22 | Serial transport (`System.IO.Ports` wrapper, async read loop into a ring buffer, RS-232/485 line settings from the connection string) | transports/serial |
| C-23 | Modbus RTU driver (`ModbusRtuConnection` — slave address + PDU + CRC-16 framing over the serial transport; Read/Write; CRC and RTU-connection unit tests via a scripted transport) | drivers/modbus |

### Not yet implemented

| Gap | Description |
| --- | --- |
| GAP-1 | ~~Serial transport hardware verification~~ — **done 2026-09-19**. `SerialTransportInstance`'s receive loop was rewritten to poll `BytesToRead` and read synchronously (was an async `BaseStream.ReadAsync(ct)` loop) and ran against a real RS-485 link, delivering bytes correctly — see GAP-2. Still no unit test of the wrapper in isolation. |
| GAP-2 | ~~Modbus RTU hardware verification~~ — **done 2026-09-19** at the wire and framing level: raw frame exchange and the `ModbusRtuConnection` driver read both correct against a Mitsubishi QJ71C24N on a real RS-485 link, see `docs/hardware-verification.md`. Scope limit: that rig is a **fixed-response stand-in** — its ladder answers every request identically regardless of function code, address or quantity — so slave-side address-range and exception-code handling remain unverified against hardware (they are covered by scripted-transport tests only). An earlier Siemens S7-1214C + CM 1241 attempt was retired after its RS-485 bench link never produced a full round trip (isolated to the link, not the driver). |
...
| GAP-5 | ~~S7 hardware verification~~ — **done**, against a Siemens S7-1214C (DC/DC/DC): PASS 12/12 on 2026-09-03, expanded to PASS 50/50 on 2026-09-16 plus a persistent I/Q/M/DB matrix (43/43) with independent read-back over new connections (25/25). See `docs/hardware-verification.md`. |
...
| GAP-8 | Generator coverage. The generator emits working parse/serialize and both Modbus and S7 round-trip the shared vectors. Handled: all field types except `manualArray` / `peek` / `assert` / `abstract` / `unknown`; discriminated types including a dotted discriminator (`parameter.parameterType`), a `simple` field that is also a discriminator, prefix- and suffix-field inheritance through the constructor (S7's `COTPPacket` trailer), pre-`typeSwitch` context passed to a child's `StaticParse` (S7's `parameterLength` / `payloadLength` / `headerLength`); `curPos` relative to a type's start; `_lastItem` threaded into an array element (S7's `S7VarPayloadDataItem` padding); parameterised enums including a self-reference, `'null'` values (nullable accessor), char / string-literal values, and an `enum` field keyed on an attribute (`TransportSize … code`); repeated-value enums (deduplicated switch arms); bare enum-constant references; `CAST` / `COUNT` / `CEIL` / `ARRAY_SIZE_IN_BYTES` / `STATIC_CALL`. `dataIo` types are emitted as a static `StaticParse` / `StaticSerialize` / `GetLengthInBits` over `IPlcValue` (S7's `DataItem`, Modbus's `DataItem`). S7's `DataItem` is fully generated - the scalar cases (`bit` / `int` / `uint` / `float` widths → `Plc{width}`), `CHAR` / `WCHAR`, `STRING` / `WSTRING` (via the ported `parseS7String` / `serializeS7String`), and the whole TIA date / time family (`TIME` / `S5TIME` / `LTIME` / `DATE` / `TIME_OF_DAY` / `LTIME_OF_DAY` / `DATE_AND_TIME` / `DATE_AND_LTIME` / `DTL`, via the ported `parseS5Time` / `parseTiaDate` / `parseSiemensYear` and a BCD helper). `IPlcValue` grew `GetDuration` / `GetDate` / `GetTime`, `PlcLTIME` / `PlcTIME_OF_DAY` / `PlcDATE` were re-based on `TimeSpan` / `TimeOnly` / `DateOnly`, and `PlcLTIME_OF_DAY` / `PlcDATE_AND_LTIME` are new (nanosecond-exact). `vstring` maps to `string`, honouring an inline length expression (`vstring 'stringLength * 8' value`). Modbus's multi-value `numberOfValues > 1` cases parse to a `PlcList` of the wrapped primitive (`bit → PlcBOOL`, `int 16 → PlcINT`, `string 8 → PlcCHAR`, …), one element per count, and honour the `bigEndian` argument: a `false` frame's multi-byte values are byte-swapped (`BinaryPrimitives.ReverseEndianness`), which plc4j leaves to the driver's buffer context. Two deliberate departures from plc4j's `DataItem`, both in fields a device consumes but the parser discards: the `DATE_AND_TIME` / `DTL` `dayOfWeek` is written Sunday=1..Saturday=7 (the mspec's own DTL comment and the Siemens doc) where plc4j writes ISO Monday=1..Sunday=7; and a few malformed-frame paths (a `DATE` before 1990, a `DATE_AND_TIME` year past 2089, a non-canonical `S5TIME`) are carried through the same way plc4j carries them rather than rejected. `GetLengthInBits` for `S7_S5TIME` returns 2 (the mspec's `'2'`, a plc4j-shared quirk - it should be 16); no caller sizes an S5TIME item today. Not yet: a `byteOrder='"LITTLE_ENDIAN"'` type attribute (no Modbus / S7 type declares one; only the dataIo `bigEndian` path swaps today). KNX generation is complete and the output compiles. `MspecReader` rewrites the empty-string literal `''` (unrepresentable in the shared expression grammar); an `external='true'` enum (`PlcValueType`) is left to the SPI rather than shadowed by an empty stub; a hyphenated `vstring` id (`DPST-1-1`, which lexes as `DPST - 1 - 1`) and a bare-number value for a `string` parameter are folded back to the string literal they spell; a `dataIo` case with no discriminator becomes the `else` branch (no unreachable fallback `return`); `float 16` (KNX DPT 9.x) reads and writes through the buffer's KNXFloat path. `KnxDatapoint`'s and `KnxProperty`'s struct cases are generated as a `PlcStruct` keyed by field name - a fixed-count `byte` array field becomes a `PlcRawByteArray` - matching plc4j (`PlcStruct` and `PlcRawByteArray` gained the accessors and content equality they were missing). The full `knxnetip` model (180 files, `KnxDatapointType`'s 1882 rows included) is now the checked-in model - it replaced the Java-plugin output and CI drift-checks it alongside Modbus and S7. The KNXnet/IP driver drives it; the handshake and tunnelling frames round-trip against a scripted gateway. Still stubbed: 7 of `KnxDatapoint`'s own `TIME` / `LTIME` / `DATE` / `TIME_OF_DAY` / `DATE_AND_TIME` cases - the mspec marks each `// TODO: Check if this is correct` and nothing consumes them. |
| GAP-9 | NuGet **publishing**. Packaging works: every library project sets `GeneratePackageOnBuild`, `Directory.Build.props` carries the shared package metadata, and `dotnet pack plc4net.sln` → `dotnet nuget push -s <folder>` → `<PackageReference>` from a separate project is a verified round trip (see `docs/packaging.md`). The `s7-verify` / `modbus-verify` harnesses pack as `dotnet tool`s. Not done: publishing to nuget.org. That is a PMC decision and goes through the ASF release process — a release vote on `dev@`, a release manager and signed artifacts — not something a contributor does; the version tracks the Maven reactor (`1.1.0-SNAPSHOT` in `Directory.Build.props`) until a release is cut. |
...
| --- | --- | --- |
| P0 | CI first-run approval | Awaiting a committer to approve the workflow run; no check run has ever executed on PR #2656, so every build/test result reported there is local-only |
| P0 | Decompose PR #2656 into reviewable slices | #2656 stays a draft umbrella/spike. Slices land as separate PRs against `develop`, in dependency order: build infrastructure + api + spi → the mspec generator → transports → one per driver, each driver's generated model travelling with it |
...
first when commissioning a panel. **Items 1–3 and 5 have since landed** (serial
transport, Modbus RTU driver, S7 Write, and the S7 hardware verification below);
they are kept here for context.
...
| 5 | **S7 hardware verification** | — | Done 2026-09-03 — end-to-end read/write against a real Siemens S7-1214C (DC/DC/DC), PASS 12/12. |
...
| Item | Description |
| --- | --- |
| Generator coverage (GAP-8) | The generator emits parse/serialize/length for every type; the `DataItem` `dataIo`s of S7 and Modbus are fully generated (scalar, string, TIA date / time, multi-value `PlcList`, little-endian) and round-trip; `vstring` with an inline length works; the KNXnet/IP model is generated, checked in, CI drift-checked, and driven by the KNXnet/IP driver (`''` rewrite, external `PlcValueType`, hyphenated `DPST-1-1` ids, `PlcStruct` / `PlcRawByteArray` struct cases, `float 16`), with 7 unconsumed `KnxDatapoint` `TIME` / `DATE` cases stubbed. Remaining: the `peek` / `assert` / `manualArray` field families (unused by Modbus / S7 / KNX), a `byteOrder` type attribute. Then move the hand-written Modbus / S7 codecs onto the generated models. |
