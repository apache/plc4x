...
├── test/                                             (test projects)
│   ├── spi-test/               plc4net-spi-test      (336 xUnit tests)
...
| Item | Description |
| --- | --- |
| Generator coverage (GAP-8) | The generator emits parse/serialize/length for every type; S7's and Modbus's `DataItem` `dataIo`s are fully generated (scalar, string, TIA date / time, multi-value `PlcList`, little-endian) and round-trip; `vstring` with an inline length works; KNX generates and compiles end to end (`''` rewrite, external `PlcValueType`, hyphenated `DPST-1-1` ids, `PlcStruct` / `PlcRawByteArray` struct cases, `float 16`), with 7 unconsumed `KnxDatapoint` `TIME` / `DATE` cases stubbed. Remaining: the `peek` / `assert` / `manualArray` field families (unused by Modbus / S7), a `byteOrder` type attribute, swapping the checked-in KNX model for the generated one against frame vectors. Then move the hand-written Modbus / S7 codecs onto the generated models. |
