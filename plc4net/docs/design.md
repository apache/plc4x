...
| Item | Description |
| --- | --- |
| Generator coverage (GAP-8) | The generator emits parse/serialize/length for every type; S7's and Modbus's `DataItem` `dataIo`s are fully generated (scalar, string, TIA date / time, multi-value `PlcList`, little-endian) and round-trip; `vstring` with an inline length works. Remaining: the `peek` / `assert` / `manualArray` field families (unused by Modbus / S7), a `byteOrder` type attribute, and full KNX — `''` now parses but `KnxDatapoint` needs `PlcStruct` `dataIo` emission for its struct cases, hyphenated `vstring` ids, and `KnxDatapointType` to compile. Then move the hand-written Modbus / S7 codecs onto the generated models. |
