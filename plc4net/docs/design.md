...
├── test/                                             (test projects)
│   ├── spi-test/               plc4net-spi-test      (344 xUnit tests)
...
| GAP-2 | ~~Modbus RTU hardware verification~~ — **done 2026-09-19**, verified against a Mitsubishi QJ71C24N (non-procedure communication acting as a fixed-response slave): raw frame exchange and the `ModbusRtuConnection` driver read both correct, see `docs/modbus-hardware-report.md`. An earlier Siemens S7-1214C + CM 1241 attempt was dropped after its RS-485 bench link never produced a full round trip (isolated to the link, not the driver). |
