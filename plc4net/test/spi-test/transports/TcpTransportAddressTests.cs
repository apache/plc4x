    ...
    public class TcpTransportAddressTests
    {
        ...
        public void Parses_host_and_port()
        {
            var (host, port, driverConfig) =
                TcpTransport.ParseAddress("192.168.0.1:102", -1);
        ...
        {
            var (host, port, _) =
                TcpTransport.ParseAddress("192.168.0.1", 502);
        ...
        public void Host_without_port_and_no_default_throws()
        {
            Assert.Throws<TransportException>(
                () => TcpTransport.ParseAddress("192.168.0.1", -1));
        }
        ...
        {
            var (host, port, _) =
                TcpTransport.ParseAddress("[::1]:502", -1);
        ...
        {
            var (host, port, driverConfig) =
                TcpTransport.ParseAddress("localhost:4840/milo", -1);
        ...
        {
            Assert.Throws<TransportException>(
                () => TcpTransport.ParseAddress("host:99999", -1));
        }
    }
}