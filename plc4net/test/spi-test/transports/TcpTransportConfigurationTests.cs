...
namespace org.apache.plc4net.spi.test.transports
{
    ...
    /// </summary>
    /// <remarks>
    /// The prefixing rule comes from the Java side: the transport declares its
    /// options via <c>@ConfigurationParameter</c> and
    /// <c>ConfigurationFactory.createPrefixedConfiguration</c> builds the prefix from
    /// the transport code, then strips it before matching. So an option named
    ...
    /// with the same word, not a typo. The option names themselves are this port's
    /// own; the Java SPI3 transport spells them "no-delay" and "connect-timeout-ms".
    /// </remarks>
