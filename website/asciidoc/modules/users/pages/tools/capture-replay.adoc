...
Here simply look for the `target/plc4j-capture-replay-{current-project-version}-uber-jar.jar` and run:

[subs=attributes+]
----
   java -jar target/plc4j-capture-replay-{current-project-version}-uber-jar.jar --input-file {path to capture} --output-device {name of the network device} --replay-speed 1 --loop true
----
