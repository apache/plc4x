...
[graphviz,plc4x-server-architecture,svg,opts="inline",svg-type="inline"]
----
...
* `tls.verify=false` disables certificate validation — needed when the server uses an auto-generated self-signed certificate. With a properly trusted (CA-signed) certificate, leave it at its default (`true`).
...
| TLS cert not trusted by the client. Add `tls.verify=false`, or trust/pin the server cert.
