...
It opens PLC4X connections to any number of devices, and publishes the tags configured for them as an OPC UA address
space, so that any OPC UA client can read and write them - no matter which protocol the device itself speaks.

The server is built on the https://projects.eclipse.org/projects/iot.milo[Eclipse Milo] OPC UA stack.

It lives in the https://github.com/apache/plc4x-extras/tree/develop/plc4j/tools/opcua-server[plc4x-extras] repository.

NOTE: Do not confuse this with the xref:protocols/opcua.adoc[OPC UA driver], which lets a PLC4X application act as an
OPC UA _client_.
...
This creates the runnable fat-jar `target/plc4x-opcua-server-{current-project-version}-uber-jar.jar`.
...
[subs=attributes+]
----
java -jar plc4x-opcua-server-{current-project-version}-uber-jar.jar -c \{path-to-config-file} -i
----
...
[subs=attributes+]
----
java -jar plc4x-opcua-server-{current-project-version}-uber-jar.jar -c \{path-to-config-file} -i -s
----
