...
Array indices may appear at any position of the path, not only at the end. Symbolic addresses are
resolved against the symbol- and data-type-table, so they require
`load-symbol-and-data-type-tables` to be enabled (the default).
...
the `load-symbol-and-data-type-tables` setting.
