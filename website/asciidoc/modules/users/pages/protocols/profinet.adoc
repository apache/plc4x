...
WARNING: The PROFINET driver is not usable yet. Two parallel implementations exist in the code base
(`plc4j-driver-profinet` and `plc4j-driver-profinet-ng`, of which only the latter is shipped) and
neither completes a working exchange - the subscription response future is never completed and
cyclic IO frames are logged rather than dispatched. This page describes the intended design and is
kept for reference; it is not linked from the navigation.
...
|| `discover` |Locates PROFINET devices on the network segment via DCP.

|| `browse` |Lists the submodules of a connected device as tags.

|| `subscribe` |*Does not deliver data.* The connect handshake is performed, but the subscription
response future is never completed and `ProfinetConnection.onRegisterConsumer` throws
`UnsupportedOperationException`, so no cyclic IO ever reaches an application.
