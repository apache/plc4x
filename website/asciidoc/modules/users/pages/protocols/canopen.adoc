...
On application side, for convenience, you can use `NMTStateRequest` enumeration instead of plain numbers.
Be aware that passed number is parsed as an enum, so vendor customization on unused bits are not supported.
