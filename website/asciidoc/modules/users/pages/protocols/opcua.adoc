...
* `trust-store-file` - validate the certificate chain against the given trust store. The acceptance
...
WARNING: `insecure-certificate-verification=true` disables server certificate verification
altogether. This makes the connection vulnerable to man-in-the-middle attacks and is intended for
local testing only.

=== Client certificate

If no `key-store-file` is configured, the driver generates a self-signed application instance
...
certificate through `key-store-file` instead - see
link:../getting-started/opcua-client-certificate.html[the client certificate tutorial].
...
NOTE: This is not the same certificate as the one configured through `key-store-file`. That one is
