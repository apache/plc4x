...
Each Modbus flavor is registered separately: `RegisterModbusTcpDriver`, `RegisterModbusRtuDriver`
and `RegisterModbusAsciiDriver`. The two serial flavors speak their own framing and register the
serial transport themselves, so use the one that matches the device you are talking to.
