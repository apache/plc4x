...
They can be built from source from the latest https://github.com/apache/plc4x-extras/tree/develop/plc4j/integrations/apache-kafka[plc4x extras - kafka integration] and plc4x xref:download.adoc[release] of
...
3) Copy the `target/plc4j-apache-kafka-{current-project-version}-uber-jar.jar` to the Kafka `libs` or plugin directory specified
...
example https://github.com/apache/plc4x-extras/blob/develop/plc4j/integrations/apache-kafka/config/plc4x-source.properties[config/plc4x-source.properties] file. These will need to be in JSON format and included with a couple of headers.
...
See https://github.com/apache/plc4x-extras/blob/develop/plc4j/integrations/apache-kafka/config/plc4x-sink.properties[config/sink.properties] for an example configuration.
