...
PLC4X supports both NiFi generations, in two separate sets of modules:

[cols="1,1,1"]
|===
|NiFi generation |Module |`nar` artifact

|NiFi 1.x |`plc4j/integrations/apache-nifi/nifi-1` |`plc4j-nifi-plc4x-nar`
|NiFi 2.x |`plc4j/integrations/apache-nifi/nifi-2` |`plc4j-nifi-2-plc4x-nar`
|===

== Setting Up NiFi

Use the Java version the NiFi release you are running requires - NiFi 1.x runs on Java 8, 11, 17 or 21, NiFi 2.x
...
You can either build the `nar` yourself from the https://github.com/apache/plc4x-extras/tree/develop/plc4j/integrations/apache-nifi[plc4x-extras] repository, and pick it up from the `target` directory of the matching `nar` module:

* NiFi 1.x: `plc4j/integrations/apache-nifi/nifi-1/nifi-plc4x-nar/target`
* NiFi 2.x: `plc4j/integrations/apache-nifi/nifi-2/nifi-2-plc4x-nar/target`

Or you can download a released version from Maven central: https://central.sonatype.com/search?q=plc4j-nifi-plc4x-nar[plc4j-nifi-plc4x-nar] / https://central.sonatype.com/search?q=plc4j-nifi-2-plc4x-nar[plc4j-nifi-2-plc4x-nar]
...
The `PLC connection String` is just a normal PLC4X connection string. Please consult the documentation on using the different types of supported connections xref:protocols/index.adoc[Here]

The `PLC resource address String` is a `;`-separarated list of `name-value-pairs`, where each `name` is assigned a `plc4x addrress`.

[NOTE]
====
For information on how an address string looks for the type of driver you are using, please check the documentation for the driver you are using xref:protocols/index.adoc[Here]
====
