...
|Tools
|`git`, `docker`, `gpg`, `svn`, `curl`, `java` and `shasum` are installed, and the Docker daemon is actually running.

|Git
|`user.name` and `user.email` are configured, the working tree is clean, and `origin` accepts your credentials for pushing (a `git push --dry-run`, which authenticates without changing anything).
