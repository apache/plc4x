...
So please check xref:release/release.adoc[here] (Chapters: `Preparing your system for being able to release` and `Preparing the codebase for a release`)
...
So if currently the project version in develop is `{{current-project-version}}`, we create a branch `release/\{code-generation-short-version}`.
...
So in case of preparing the release branch for `{{current-project-version}}` the command would be the following:
...
2. The name of the tag the release commit will be tagged with in the SCM (Name it `releases/code-generation/\{release-version}` (`releases/code-generation/{{current-project-version}}` in our case)
...
4. Commit the changes (commit message: `[maven-release-plugin] prepare release releases/code-generation/{{current-project-version}}`)
5. Push the commit
6. Tag the commit
...
It is important that the commit with the message "[maven-release-plugin] prepare release releases/code-generation/{{current-project-version}}" is tagged with the release tag (in this case releases/code-generation/{{current-project-version}})
...
----
   ./KEYS
   ./build-tools/code-generation/{{current-project-version}}
   ./build-tools/code-generation/{{current-project-version}}/rc1
   ./build-tools/code-generation/{{current-project-version}}/rc1/README
   ./build-tools/code-generation/{{current-project-version}}/rc1/RELEASE_NOTES
   ./build-tools/code-generation/{{current-project-version}}/rc1/apache-plc4x-code-generation-{{current-project-version}}-source-release.zip
   ./build-tools/code-generation/{{current-project-version}}/rc1/apache-plc4x-code-generation-{{current-project-version}}-source-release.zip.asc
   ./build-tools/code-generation/{{current-project-version}}/rc1/apache-plc4x-code-generation-{{current-project-version}}-source-release.zip.sha512
----

I usually prepare exactly the same directory structure, starting with the {{current-project-version}} locally and then just import everything using the following command:

[subs="verbatim,attributes"]
----
svn import {{current-project-version}} https://dist.apache.org/repos/dist/dev/plc4x/build-tools/code-generation/{{current-project-version}} -m"Staging of rc1 of PLC4X Build-Tools (Code-Generation) {{current-project-version}}"
----
...
----
E-Mail Topic:
[VOTE] Apache PLC4X Build-Tools Code-Generation {{current-project-version}} RC1

Message:
Apache PLC4X Build-Tools Code-Generation {{current-project-version}} has been staged under [2]
...
Release tag: releases/code-generation/{{current-project-version}}
...
[2] https://dist.apache.org/repos/dist/dev/plc4x/build-tools/code-generation/{{current-project-version}}/rc1/
...
----
E-Mail Topic:
[DISCUSS] Apache PLC4X Build-Tools Code-Generation {{current-project-version}} RC1
...
----
E-Mail Topic:
[RESULT] [VOTE] Apache PLC4X Build-Tools Code-Generation {{current-project-version}} RC1
...
[subs="verbatim,attributes"]
----
   svn move -m "Release Apache PLC4X {{current-project-version}}" \
       https://dist.apache.org/repos/dist/dev/plc4x/build-tools/code-generation/{{current-project-version}}/rc1 \
       https://dist.apache.org/repos/dist/release/plc4x/build-tools/code-generation/{{current-project-version}}
----
...
This can be done like this:
    ...
    svn delete https://dist.apache.org/repos/dist/release/plc4x/build-tools/code-generation/{1.8.0}/ -m"deleted old version of the build-tools"

After this, https://dist.apache.org/repos/dist/release/plc4x/build-tools/code-generation should only contain the latest release directory.
...
git merge releases/code-generation/{{current-project-version}}
----
...
[subs="verbatim,attributes"]
----
git merge -X theirs releases/code-generation/{{current-project-version}}
----
