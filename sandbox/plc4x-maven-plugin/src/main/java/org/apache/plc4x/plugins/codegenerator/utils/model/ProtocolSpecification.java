...
public class ProtocolSpecification {
    ...
    public ProtocolSpecification(Document dfdlSpecification) {
        types = new HashMap<>();
        ...
        for (Node complexTypeNode : complexTypes) {
            //List<Segment> segments = parseComplexTypeSegments((Element) complexTypeNode);
    ...
    private ComplexType parseComplexType(Node complexTypeNode) {
        ...
        List<Segment> segments = parseComplexTypeSegments(complexTypeElement);
        return new ComplexType(name, segments);
    }

    private List<Segment> parseComplexTypeSegments(Element complexTypeElement) {
                ...
                case "element":
                    ...
                    if(typeName != null) {
                        if(!typeName.contains(":")) {
                            throw new RuntimeException("Namespace prefix missing");
                        }
                    ...
                    // In this case we simply add the children instead of the complex element itself.
                        ...
                        segments.addAll(parseComplexTypeSegments((Element) innerComplexTypeNode));
                    }
                    break;
                case "choice":
    ...
    public List<QName> getComplexTypeNames() {
        List<QName> complexTypeNames = new LinkedList<>();
        for (Map.Entry<QName, Type> typeEntry : types.entrySet()) {
            if(typeEntry.getValue() instanceof ComplexType) {
                complexTypeNames.add(typeEntry.getKey());
            }
        }
        return complexTypeNames;
    }

   public List<Segment> getSegments(QName typeName) {
       Type type = types.get(typeName);
       if(type instanceof ComplexType) {
           ComplexType complexType = (ComplexType) type;
           return complexType.getSegments();
       }
