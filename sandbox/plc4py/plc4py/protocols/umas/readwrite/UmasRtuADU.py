...
@dataclass
class UmasRtuADU(UmasADU):
    address: int
    pdu: UmasPDU
    # Arguments.
    response: bool
    # Accessors for discriminator values.
    driver_type: DriverType = DriverType.Umas_RTU

    def serialize_umas_adu_child(self, write_buffer: WriteBuffer):
        write_buffer.push_context("UmasRtuADU")
        ...
        write_buffer.pop_context("UmasRtuADU")
    ...
    def static_parse_builder(
        ...
        read_buffer.push_context("UmasRtuADU")
        ...
        read_buffer.pop_context("UmasRtuADU")
        # Create the instance
        return UmasRtuADUBuilder(address, pdu)

    def equals(self, o: object) -> bool:
        if self == o:
            return True

        if not isinstance(o, UmasRtuADU):
            return False

        that: UmasRtuADU = UmasRtuADU(o)
        return (
...
@dataclass
class UmasRtuADUBuilder(UmasADUBuilder):
    address: int
    pdu: UmasPDU
    ...
    def build(
        ...
        umas_rtu_adu: UmasRtuADU = UmasRtuADU(response, self.address, self.pdu)
        return umas_rtu_adu
