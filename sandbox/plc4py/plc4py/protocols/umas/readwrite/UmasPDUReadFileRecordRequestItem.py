...
@dataclass
class UmasPDUReadFileRecordRequestItem:
    reference_type: int
    file_number: int
    record_number: int
    record_length: int
    ...
    def serialize(self, write_buffer: WriteBuffer):
        write_buffer.push_context("UmasPDUReadFileRecordRequestItem")
        ...
        write_buffer.pop_context("UmasPDUReadFileRecordRequestItem")
    ...
    def length_in_bits(self) -> int:
        ...
        _value: UmasPDUReadFileRecordRequestItem = self
    ...
    def static_parse(read_buffer: ReadBuffer, **kwargs):
        return UmasPDUReadFileRecordRequestItem.static_parse_context(read_buffer)

    @staticmethod
    def static_parse_context(read_buffer: ReadBuffer):
        read_buffer.push_context("UmasPDUReadFileRecordRequestItem")
        ...
        read_buffer.pop_context("UmasPDUReadFileRecordRequestItem")
        # Create the instance
        _umas_pdu_read_file_record_request_item: UmasPDUReadFileRecordRequestItem = (
            UmasPDUReadFileRecordRequestItem(
                reference_type, file_number, record_number, record_length
            )
        )
        return _umas_pdu_read_file_record_request_item

    def equals(self, o: object) -> bool:
        if self == o:
            return True

        if not isinstance(o, UmasPDUReadFileRecordRequestItem):
            return False

        that: UmasPDUReadFileRecordRequestItem = UmasPDUReadFileRecordRequestItem(o)
        return (
