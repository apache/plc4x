...
type MQTT_ControlPacket_SUBACK struct {
	*MQTT_ControlPacket
...
type IMQTT_ControlPacket_SUBACK interface {
...
///////////////////////////////////////////////////////////
func (m *MQTT_ControlPacket_SUBACK) PacketType() MQTT_ControlPacketType {
	return MQTT_ControlPacketType_SUBACK
}


func (m *MQTT_ControlPacket_SUBACK) InitializeParent(parent *MQTT_ControlPacket) {
}

func NewMQTT_ControlPacket_SUBACK(remainingLength uint8, packetIdentifier uint16, propertyLength *uint32, properties []*MQTT_Property, results []MQTT_ReasonCode) *MQTT_ControlPacket {
	child := &MQTT_ControlPacket_SUBACK{
...
func CastMQTT_ControlPacket_SUBACK(structType interface{}) *MQTT_ControlPacket_SUBACK {
	castFunc := func(typ interface{}) *MQTT_ControlPacket_SUBACK {
		if casted, ok := typ.(MQTT_ControlPacket_SUBACK); ok {
			return &casted
		}
		if casted, ok := typ.(*MQTT_ControlPacket_SUBACK); ok {
			return casted
		...
		if casted, ok := typ.(MQTT_ControlPacket); ok {
			return CastMQTT_ControlPacket_SUBACK(casted.Child)
		}
		if casted, ok := typ.(*MQTT_ControlPacket); ok {
			return CastMQTT_ControlPacket_SUBACK(casted.Child)
		}
...
func (m *MQTT_ControlPacket_SUBACK) GetTypeName() string {
	return "MQTT_ControlPacket_SUBACK"
}

func (m *MQTT_ControlPacket_SUBACK) LengthInBits() uint16 {
	return m.LengthInBitsConditional(false)
}

func (m *MQTT_ControlPacket_SUBACK) LengthInBitsConditional(lastItem bool) uint16 {
...
func (m *MQTT_ControlPacket_SUBACK) LengthInBytes() uint16 {
	return m.LengthInBits() / 8
}

func MQTT_ControlPacket_SUBACKParse(readBuffer utils.ReadBuffer) (*MQTT_ControlPacket, error) {
	if pullErr := readBuffer.PullContext("MQTT_ControlPacket_SUBACK"); pullErr != nil {
		return nil, pullErr
	}
...
_packetIdentifier, _packetIdentifierErr := readBuffer.ReadUint16("packetIdentifier", 16)
	...
	if closeErr := readBuffer.CloseContext("MQTT_ControlPacket_SUBACK"); closeErr != nil {
		return nil, closeErr
	}

	// Create a partially initialized instance
	_child := &MQTT_ControlPacket_SUBACK{
		RemainingLength: remainingLength,
		PacketIdentifier: packetIdentifier,
		PropertyLength: propertyLength,
		Properties: properties,
...
func (m *MQTT_ControlPacket_SUBACK) Serialize(writeBuffer utils.WriteBuffer) error {
	ser := func() error {
		if pushErr := writeBuffer.PushContext("MQTT_ControlPacket_SUBACK"); pushErr != nil {
			return pushErr
		}
	...
	if m.Results != nil {
		...
		if popErr := writeBuffer.PopContext("MQTT_ControlPacket_SUBACK"); popErr != nil {
			return popErr
		}
		return nil
	}
...
func (m *MQTT_ControlPacket_SUBACK) String() string {
	if m == nil {
		return "<nil>"
	}
