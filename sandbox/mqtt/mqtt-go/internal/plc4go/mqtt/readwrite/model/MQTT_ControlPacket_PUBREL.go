...
type MQTT_ControlPacket_PUBREL struct {
	*MQTT_ControlPacket
...
type IMQTT_ControlPacket_PUBREL interface {
...
///////////////////////////////////////////////////////////
func (m *MQTT_ControlPacket_PUBREL) PacketType() MQTT_ControlPacketType {
	return MQTT_ControlPacketType_PUBREL
}


func (m *MQTT_ControlPacket_PUBREL) InitializeParent(parent *MQTT_ControlPacket) {
}

func NewMQTT_ControlPacket_PUBREL(remainingLength uint8, packetIdentifier uint16, reasonCode *MQTT_ReasonCode, propertyLength *uint32, properties []*MQTT_Property) *MQTT_ControlPacket {
	child := &MQTT_ControlPacket_PUBREL{
...
func CastMQTT_ControlPacket_PUBREL(structType interface{}) *MQTT_ControlPacket_PUBREL {
	castFunc := func(typ interface{}) *MQTT_ControlPacket_PUBREL {
		if casted, ok := typ.(MQTT_ControlPacket_PUBREL); ok {
			return &casted
		}
		if casted, ok := typ.(*MQTT_ControlPacket_PUBREL); ok {
			return casted
		...
		if casted, ok := typ.(MQTT_ControlPacket); ok {
			return CastMQTT_ControlPacket_PUBREL(casted.Child)
		}
		if casted, ok := typ.(*MQTT_ControlPacket); ok {
			return CastMQTT_ControlPacket_PUBREL(casted.Child)
		}
...
func (m *MQTT_ControlPacket_PUBREL) GetTypeName() string {
	return "MQTT_ControlPacket_PUBREL"
}

func (m *MQTT_ControlPacket_PUBREL) LengthInBits() uint16 {
	return m.LengthInBitsConditional(false)
}

func (m *MQTT_ControlPacket_PUBREL) LengthInBitsConditional(lastItem bool) uint16 {
...
func (m *MQTT_ControlPacket_PUBREL) LengthInBytes() uint16 {
	return m.LengthInBits() / 8
}

func MQTT_ControlPacket_PUBRELParse(readBuffer utils.ReadBuffer) (*MQTT_ControlPacket, error) {
	if pullErr := readBuffer.PullContext("MQTT_ControlPacket_PUBREL"); pullErr != nil {
		return nil, pullErr
	}
...
_packetIdentifier, _packetIdentifierErr := readBuffer.ReadUint16("packetIdentifier", 16)
	...
	if closeErr := readBuffer.CloseContext("MQTT_ControlPacket_PUBREL"); closeErr != nil {
		return nil, closeErr
	}

	// Create a partially initialized instance
	_child := &MQTT_ControlPacket_PUBREL{
		RemainingLength: remainingLength,
		PacketIdentifier: packetIdentifier,
		ReasonCode: reasonCode,
		PropertyLength: propertyLength,
...
func (m *MQTT_ControlPacket_PUBREL) Serialize(writeBuffer utils.WriteBuffer) error {
	ser := func() error {
		if pushErr := writeBuffer.PushContext("MQTT_ControlPacket_PUBREL"); pushErr != nil {
			return pushErr
		}
	...
	if m.Properties != nil {
		...
		if popErr := writeBuffer.PopContext("MQTT_ControlPacket_PUBREL"); popErr != nil {
			return popErr
		}
		return nil
	}
...
func (m *MQTT_ControlPacket_PUBREL) String() string {
	if m == nil {
		return "<nil>"
	}
