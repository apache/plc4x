...
type MQTT_ControlPacket_UNSUBSCRIBE struct {
	*MQTT_ControlPacket
...
type IMQTT_ControlPacket_UNSUBSCRIBE interface {
...
///////////////////////////////////////////////////////////
func (m *MQTT_ControlPacket_UNSUBSCRIBE) PacketType() MQTT_ControlPacketType {
	return MQTT_ControlPacketType_UNSUBSCRIBE
}


func (m *MQTT_ControlPacket_UNSUBSCRIBE) InitializeParent(parent *MQTT_ControlPacket) {
}

func NewMQTT_ControlPacket_UNSUBSCRIBE(remainingLength uint8, packetIdentifier uint16, propertyLength *uint32, properties []*MQTT_Property, filters []*MQTT_String) *MQTT_ControlPacket {
	child := &MQTT_ControlPacket_UNSUBSCRIBE{
...
func CastMQTT_ControlPacket_UNSUBSCRIBE(structType interface{}) *MQTT_ControlPacket_UNSUBSCRIBE {
	castFunc := func(typ interface{}) *MQTT_ControlPacket_UNSUBSCRIBE {
		if casted, ok := typ.(MQTT_ControlPacket_UNSUBSCRIBE); ok {
			return &casted
		}
		if casted, ok := typ.(*MQTT_ControlPacket_UNSUBSCRIBE); ok {
			return casted
		...
		if casted, ok := typ.(MQTT_ControlPacket); ok {
			return CastMQTT_ControlPacket_UNSUBSCRIBE(casted.Child)
		}
		if casted, ok := typ.(*MQTT_ControlPacket); ok {
			return CastMQTT_ControlPacket_UNSUBSCRIBE(casted.Child)
		}
...
func (m *MQTT_ControlPacket_UNSUBSCRIBE) GetTypeName() string {
	return "MQTT_ControlPacket_UNSUBSCRIBE"
}

func (m *MQTT_ControlPacket_UNSUBSCRIBE) LengthInBits() uint16 {
	return m.LengthInBitsConditional(false)
}

func (m *MQTT_ControlPacket_UNSUBSCRIBE) LengthInBitsConditional(lastItem bool) uint16 {
...
func (m *MQTT_ControlPacket_UNSUBSCRIBE) LengthInBytes() uint16 {
	return m.LengthInBits() / 8
}

func MQTT_ControlPacket_UNSUBSCRIBEParse(readBuffer utils.ReadBuffer) (*MQTT_ControlPacket, error) {
	if pullErr := readBuffer.PullContext("MQTT_ControlPacket_UNSUBSCRIBE"); pullErr != nil {
		return nil, pullErr
	}
...
_packetIdentifier, _packetIdentifierErr := readBuffer.ReadUint16("packetIdentifier", 16)
	...
	curPos = readBuffer.GetPos() - startPos
	// Count array
	filters := make([]*MQTT_String, uint16(remainingLength) - uint16(curPos))
	{
		for curItem := uint16(0); curItem < uint16(uint16(remainingLength) - uint16(curPos)); curItem++ {
			_item, _err := MQTT_StringParse(readBuffer)
			if _err != nil {
	...
	if closeErr := readBuffer.CloseContext("MQTT_ControlPacket_UNSUBSCRIBE"); closeErr != nil {
		return nil, closeErr
	}

	// Create a partially initialized instance
	_child := &MQTT_ControlPacket_UNSUBSCRIBE{
		RemainingLength: remainingLength,
		PacketIdentifier: packetIdentifier,
		PropertyLength: propertyLength,
		Properties: properties,
...
func (m *MQTT_ControlPacket_UNSUBSCRIBE) Serialize(writeBuffer utils.WriteBuffer) error {
	ser := func() error {
		if pushErr := writeBuffer.PushContext("MQTT_ControlPacket_UNSUBSCRIBE"); pushErr != nil {
			return pushErr
		}
	...
	if m.Filters != nil {
		...
		if popErr := writeBuffer.PopContext("MQTT_ControlPacket_UNSUBSCRIBE"); popErr != nil {
			return popErr
		}
		return nil
	}
...
func (m *MQTT_ControlPacket_UNSUBSCRIBE) String() string {
	if m == nil {
		return "<nil>"
	}
