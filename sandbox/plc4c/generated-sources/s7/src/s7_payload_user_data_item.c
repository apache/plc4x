...
plc4c_return_code plc4c_s7_read_write_s7_payload_user_data_item_parse(plc4c_spi_read_buffer* buf, unsigned int cpuFunctionType, plc4c_s7_read_write_s7_payload_user_data_item** _message) {
  ...
  uint16_t szlIndex = 0;
  _res = plc4c_spi_read_unsigned_int(buf, 16, (uint32_t*) &szlIndex);
  if(_res != OK) {
    return _res;
  }
  ...
  if(cpuFunctionType == 0x08) { /* S7PayloadUserDataItemCpuFunctionReadSzlResponse */
    ...
    _res = plc4c_spi_read_unsigned_int(buf, 16, (uint32_t*) &szlItemLength);
    if(_res != OK) {
      return _res;
    }
    ...
    _res = plc4c_spi_read_unsigned_int(buf, 16, (uint32_t*) &szlItemCount);
    if(_res != OK) {
      return _res;
    }
