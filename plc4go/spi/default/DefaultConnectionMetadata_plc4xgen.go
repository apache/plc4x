...
func (d *DefaultConnectionMetadata) SerializeWithWriteBuffer(ctx context.Context, writeBuffer utils.WriteBuffer) error {
	...
	if err := writeBuffer.PushContext("connectionAttributes", utils.WithRenderAsList(true)); err != nil {
		return err
	}
	for _name, elem := range d.ConnectionAttributes {
		name := _name

		if err := writeBuffer.WriteString(name, uint32(len(elem)*8), elem, utils.WithEncoding("UTF-8")); err != nil {
			return err
		}
	}
	if err := writeBuffer.PopContext("connectionAttributes", utils.WithRenderAsList(true)); err != nil {
		return err
	}

	if err := writeBuffer.WriteBit("providesReading", d.ProvidesReading); err != nil {
		return err
	}

	if err := writeBuffer.WriteBit("providesWriting", d.ProvidesWriting); err != nil {
		return err
	}

	if err := writeBuffer.WriteBit("providesSubscribing", d.ProvidesSubscribing); err != nil {
		return err
	}

	if err := writeBuffer.WriteBit("providesBrowsing", d.ProvidesBrowsing); err != nil {
		return err
	}
