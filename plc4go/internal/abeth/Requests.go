...
// sendRequestAndWait sends one request through the codec and blocks until the accepted response, an
// error, or ctx cancellation. Every outcome resolves - no code path may leave the caller waiting
// forever.
...
// sendTransactedRequestAndWait is sendRequestAndWait routed through the request transaction manager,
// which is what keeps ab-eth's one-request-at-a-time promise: the manager is created with a
// concurrency of one, so a read waits for the read before it to finish.
