...
// SubscriptionEvent is the payload delivered to a consumer when a COV notification
// arrives. address carries the BACnet origin (device-id and object-id) so the
// caller can disambiguate notifications when one consumer is registered on
