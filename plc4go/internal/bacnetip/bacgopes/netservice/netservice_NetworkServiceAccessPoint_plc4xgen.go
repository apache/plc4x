...
// Code generated by "plc4xGenerator -type=NetworkServiceAccessPoint -prefix=netservice_"; DO NOT EDIT.
...
func (d *NetworkServiceAccessPoint) Serialize() ([]byte, error) {
	if d == nil {
...
func (d *NetworkServiceAccessPoint) SerializeWithWriteBuffer(ctx context.Context, writeBuffer utils.WriteBuffer) error {
	if d == nil {
		return fmt.Errorf("(*DeviceInfoCache)(nil)")
	}
	if err := writeBuffer.PushContext("NetworkServiceAccessPoint"); err != nil {
		return err
	}
	...
	for _name, elem := range d.adapters {
	...
	if err := writeBuffer.PopContext("adapters", utils.WithRenderAsList(true)); err != nil {
		return err
	}
	if d.routerInfoCache != nil {
		if err := writeBuffer.WriteString("routerInfoCache", uint32(len(d.routerInfoCache.String())*8), d.routerInfoCache.String()); err != nil {
			return err
		}
	}
	if err := writeBuffer.PushContext("pendingNets", utils.WithRenderAsList(true)); err != nil {
		return err
	}
	for _name, elem := range d.pendingNets {
		name := fmt.Sprintf("%v", &_name)
	...
	if err := writeBuffer.PopContext("pendingNets", utils.WithRenderAsList(true)); err != nil {
		return err
	}
	if d.localAdapter != nil {
		if err := writeBuffer.WriteString("localAdapter", uint32(len(d.localAdapter.String())*8), d.localAdapter.String()); err != nil {
			return err
		}
	}
	if err := writeBuffer.PopContext("NetworkServiceAccessPoint"); err != nil {
		return err
	}
	return nil
}

func (d *NetworkServiceAccessPoint) String() string {
