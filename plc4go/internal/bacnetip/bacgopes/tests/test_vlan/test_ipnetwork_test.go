...
func NewTIPNetwork(t *testing.T, nodeCount int, addressPattern string, promiscuous bool, spoofing bool) *TIPNetwork {
	...
	for i := range nodeCount {
		nodeAddress, err := bacgopes.NewAddress(localLog, fmt.Sprintf(addressPattern, i+1))
		require.NoError(t, err)
		node, err := bacgopes.NewIPNode(localLog, nodeAddress, tn.vlan, bacgopes.WithNodePromiscuous(promiscuous), bacgopes.WithNodeSpoofing(spoofing), bacgopes.WithNodeName("node"+strconv.Itoa(i+1)))
		require.NoError(t, err)
		...
		err = bacgopes.Bind(localLog, csm, node)
		require.NoError(t, err)
...
func TestIPVLAN(t *testing.T) {
	...
	t.Run("test_send_receive", func(t *testing.T) { // Test that a node can send a message to another node.
		...
		pdu := bacgopes.NewPDU(tests.NewDummyMessage([]byte("data")...),
			bacgopes.WithPDUSource(Address("192.168.2.1:47808")),
			bacgopes.WithPDUDestination(Address("192.168.2.2:47808")),
		)
		t.Log(pdu)

		// node 1 sends the pdu, mode 2 gets it
		tnode1.GetStartState().Send(pdu, nil).Success("")
		tnode2.GetStartState().Receive(bacgopes.NewArgs((bacgopes.PDU)(nil)), bacgopes.NewKWArgs(
			bacgopes.KWPPDUSource, AddressTuple("192.168.2.1", 47808),
		)).Success("")
	...
	t.Run("test_broadcast", func(t *testing.T) { // Test that a node can send out a 'local broadcast' message which will be received by every other node.
		...
		pdu := bacgopes.NewPDU(tests.NewDummyMessage([]byte("data")...),
			bacgopes.WithPDUSource(Address("192.168.3.1:47808")),
			bacgopes.WithPDUDestination(Address("192.168.3.255:47808")),
		)
		t.Log(pdu)

		// node 1 sends the pdu, node 2 and 3 each get it
		tnode1.GetStartState().Send(pdu, nil).Success("")
		tnode2.GetStartState().Receive(bacgopes.NewArgs(bacgopes.NewPDU(nil)), bacgopes.NewKWArgs(
			bacgopes.KWPPDUSource, AddressTuple("192.168.3.1", 47808),
		)).Success("")
		tnode3.GetStartState().Receive(bacgopes.NewArgs(bacgopes.NewPDU(nil)), bacgopes.NewKWArgs(
			bacgopes.KWPPDUSource, AddressTuple("192.168.3.1", 47808),
		)).Success("")
	...
	t.Run("test_spoof_fail", func(t *testing.T) { // Test verifying that a node cannot send out packets with a source address other than its own, see also test_spoof_pass().
		...
		pdu := bacgopes.NewPDU(tests.NewDummyMessage([]byte("data")...),
			bacgopes.WithPDUSource(Address("192.168.4.2:47808")),
			bacgopes.WithPDUDestination(Address("192.168.4.3:47808")),
		)
		t.Log(pdu)
	...
	t.Run("test_spoof_pass", func(t *testing.T) { // Test allowing a node to send out packets with a source address other than its own, see also test_spoof_fail().
		...
		pdu := bacgopes.NewPDU(tests.NewDummyMessage([]byte("data")...),
			bacgopes.WithPDUSource(Address("192.168.5.3:47808")),
			bacgopes.WithPDUDestination(Address("192.168.5.1:47808")),
		)
		t.Log(pdu)
		...
		tnode1.GetStartState().
			Send(pdu, nil).
			Receive(bacgopes.NewArgs(bacgopes.NewPDU(nil)), bacgopes.NewKWArgs(
				bacgopes.KWPPDUSource, AddressTuple("192.168.5.3", 47808),
			)).
			Success("")
	...
	t.Run("test_promiscuous_pass", func(t *testing.T) { // Test 'promiscuous mode' of a node which allows it to receive every packet sent on the network.  This is like the network is a hub, or the node is connected to a 'monitor' port on a managed switch.
		...
		// make a PDU from node 1 to node 2
		src, err := bacgopes.NewAddress(testingLogger, "192.168.6.1:47808")
		require.NoError(t, err)
		dest, err := bacgopes.NewAddress(testingLogger, "192.168.6.2:47808")
		require.NoError(t, err)
		pdu := bacgopes.NewPDU(nil, bacgopes.WithPDUSource(src), bacgopes.WithPDUDestination(dest))
		t.Log(pdu)

		// node 1 sends the pdu, node 2 and 3 each get it
		tnode1.GetStartState().Send(pdu, nil).Success("")
		tnode2.GetStartState().Receive(bacgopes.NewArgs(bacgopes.NewPDU(nil)), bacgopes.NewKWArgs(
			bacgopes.KWPPDUSource, src,
		)).Success("")
		tnode3.GetStartState().Receive(bacgopes.NewArgs(bacgopes.NewPDU(nil)), bacgopes.NewKWArgs(
			bacgopes.KWPDUDestination, dest,
		)).Success("")
	...
	t.Run("test_promiscuous_fail", func(t *testing.T) {
		...
		// make a PDU from node 1 to node 2
		pdu := bacgopes.NewPDU(nil, bacgopes.WithPDUSource(Address("192.168.7.1:47808")), bacgopes.WithPDUDestination(Address("192.168.7.2:47808")))
		t.Log(pdu)

		// node 1 sends the pdu to node 2, node 3 waits and gets nothing
		tnode1.GetStartState().Send(pdu, nil).Success("")
		tnode2.GetStartState().Receive(bacgopes.NewArgs(bacgopes.NewPDU(nil)), bacgopes.NewKWArgs(
			bacgopes.KWPPDUSource, AddressTuple("192.168.7.1", 47808),
		)).Success("")
...
func (suite *RouterSuite) SetupTest() {
	...
	// make some networks
	vlan10 := bacgopes.NewIPNetwork(suite.log)
	vlan20 := bacgopes.NewIPNetwork(suite.log)

	// make a router and add the networks
	trouter := bacgopes.NewIPRouter(suite.log)
	trouter.AddNetwork(Address("192.168.10.1/24"), vlan10)
	trouter.AddNetwork(Address("192.168.20.1/24"), vlan20)

	for pattern, lan := range map[string]*bacgopes.IPNetwork{
		"192.168.10.%d/24": vlan10,
		"192.168.20.%d/24": vlan20,
	} {
		for i := range 2 {
			nodeAddress, err := bacgopes.NewAddress(suite.log, fmt.Sprintf(pattern, i+2))
			suite.NoError(err)
			...
			suite.NoError(err)
			err = bacgopes.Bind(suite.log, csm, node)
			suite.NoError(err)
...
func (suite *RouterSuite) TestSendReceive() { // Test that a node can send a message to another node.
	...
	pdu := bacgopes.NewPDU(tests.NewDummyMessage([]byte("data")...),
		bacgopes.WithPDUSource(Address("192.168.10.2:47808")),
		bacgopes.WithPDUDestination(Address("192.168.20.3:47808")))
	suite.T().Log(pdu)

	// node 1 sends the pdu, mode 2 gets it
	csm_10_2.GetStartState().Send(pdu, nil).Success("")
	csm_20_3.GetStartState().Receive(bacgopes.NewArgs(bacgopes.NewPDU(nil)), bacgopes.NewKWArgs(
		bacgopes.KWPPDUSource, AddressTuple("192.168.10.2", 47808),
	)).Success("")
...
func (suite *RouterSuite) TestLocalBroadcast() { // Test that a node can send a message to all of the other nodes on the same network.
	...
	// make a PDU from network 10 node 1 to network 20 node 2
	src, err := bacgopes.NewAddress(suite.log, "192.168.10.2:47808")
	suite.Require().NoError(err)
	dest, err := bacgopes.NewAddress(suite.log, "192.168.10.255:47808")
	suite.Require().NoError(err)
	pdu := bacgopes.NewPDU(nil, bacgopes.WithPDUSource(src), bacgopes.WithPDUDestination(dest))
	suite.T().Log(pdu)
	...
	csm_10_2.GetStartState().Send(pdu, nil).Success("")
	csm_10_3.GetStartState().Receive(bacgopes.NewArgs(bacgopes.NewPDU(nil)), bacgopes.NewKWArgs(
		bacgopes.KWPPDUSource, src,
	)).Success("")
...
func (suite *RouterSuite) TestRemoteBroadcast() { // Test that a node can send a message to all of the other nodes on a different network.
	...
	src, err := bacgopes.NewAddress(suite.log, "192.168.10.2:47808")
	require.NoError(t, err)
	dest, err := bacgopes.NewAddress(suite.log, "192.168.20.255:47808")
	require.NoError(t, err)
	pdu := bacgopes.NewPDU(nil, bacgopes.WithPDUSource(src), bacgopes.WithPDUDestination(dest))
	t.Log(pdu)
	...
	csm_10_2.GetStartState().Send(pdu, nil).Success("")
	csm_10_3.GetStartState().Timeout(1*time.Second, nil).Success("")
	csm_20_2.GetStartState().Receive(bacgopes.NewArgs(bacgopes.NewPDU(nil)), bacgopes.NewKWArgs(
		bacgopes.KWPPDUSource, src,
	)).Success("")
	csm_20_3.GetStartState().Receive(bacgopes.NewArgs(bacgopes.NewPDU(nil)), bacgopes.NewKWArgs(
		bacgopes.KWPPDUSource, src,
