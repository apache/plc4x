...
//go:generate plc4xGenerator -type=WhoIsIAmServices -prefix=device_
...
func (w *WhoIsIAmServices) IAm(address *Address) error {
	...
	maxLength := readWriteModel.MaxApduLengthAccepted_NUM_OCTETS_1476
	if w.localDevice.MaximumApduLengthAccepted != nil {
		maxLength = *w.localDevice.MaximumApduLengthAccepted
	}
	segmentationSupported := readWriteModel.BACnetSegmentation_SEGMENTED_BOTH
	if w.localDevice.SegmentationSupported != nil {
		segmentationSupported = *w.localDevice.SegmentationSupported
	}

	iAm := readWriteModel.NewBACnetUnconfirmedServiceRequestIAm(
		readWriteModel.CreateBACnetApplicationTagObjectIdentifier(ObjectIdentifierStringToTuple(w.localDevice.ObjectIdentifier)),
		readWriteModel.CreateBACnetApplicationTagUnsignedInteger(uint(maxLength)),
		readWriteModel.CreatBACnetSegmentationTagged(segmentationSupported),
		readWriteModel.CreateBACnetVendorIdApplicationTagged(w.localDevice.VendorIdentifier),
		0,
	)
