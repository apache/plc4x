...
// (see getEncodingOption), but the generated file only picks that up on a code
// generation run:
//
//	cd plc4go && mvn -Pupdate-generated-code \
//	    org.apache.plc4x.plugins:plc4x-maven-plugin:generate-driver@generate-driver-s7
//
// This test pins the CURRENT, WRONG bytes so the regeneration flips it loudly
// rather than silently. When it fails, replace the expectation with the
// wantAfterRegen vector below and delete this comment.
func TestDataItem_DateAndTimeIsStillHalfBCD(t *testing.T) {
	...
	// year is BCD (0x24 == 2024), everything after it is raw binary:
	// day 19 -> 0x13 instead of 0x19, hour 14 -> 0x0E instead of 0x14,
	// minutes 35 -> 0x23 instead of 0x35, msec 123 -> 0x07B instead of 0x123.
	currentlyWrong := []byte{0x24, 0x08, 0x13, 0x0E, 0x23, 0x07, 0x07, 0xB1}
	assert.Equal(t, currentlyWrong, serialized,
		"DataItem.go is still generated by the un-migrated data-io template")

	// What the regenerated DataItem.go will emit once every field carries the
	// encoding option, i.e. what to swap the expectation above for.
	//
	// CAUTION: this is not yet what a real S7 PLC expects. The trailing nibble is
	// the dow, and it comes from PlcDATE_AND_TIME.GetDayOfWeek, which returns the
	// ISO-8601 numbering (1 == Monday .. 7 == Sunday) that plc4j returns too and
	// that KNX DPT 19.001 wants. S7 numbers the same field 1 == Sunday ..
	// 7 == Saturday, so 2024-08-19 (a Monday) belongs on an S7 wire as dow 2, not
	// the 1 below. Rotating into the Siemens numbering is a separate S7 layer fix
	// that has to happen in the data-io template or a manual field, not in the
	// shared PlcDATE_AND_TIME value.
	wantAfterRegen := []byte{0x24, 0x08, 0x19, 0x14, 0x35, 0x07, 0x12, 0x31}
	assert.NotEqual(t, wantAfterRegen, serialized,
		"if this fails, DataItem.go was regenerated - swap the expectation above")
