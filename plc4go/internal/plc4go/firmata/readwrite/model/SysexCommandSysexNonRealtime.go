...
func SysexCommandSysexNonRealtimeParse(io utils.ReadBuffer) (*SysexCommand, error) {
	if pullErr := io.PullContext("SysexCommandSysexNonRealtime"); pullErr != nil {
		return nil, pullErr
	}

	if closeErr := io.CloseContext("SysexCommandSysexNonRealtime"); closeErr != nil {
		return nil, closeErr
	}
...
func (m *SysexCommandSysexNonRealtime) Serialize(io utils.WriteBuffer) error {
	ser := func() error {
		if pushErr := io.PushContext("SysexCommandSysexNonRealtime"); pushErr != nil {
			return pushErr
		}

		if popErr := io.PopContext("SysexCommandSysexNonRealtime"); popErr != nil {
			return popErr
		}
		return nil
	}
...
func (m *SysexCommandSysexNonRealtime) UnmarshalXML(d *xml.Decoder, start xml.StartElement) error {
	var token xml.Token
	var err error
	foundContent := false
	token = start
...
func (m *SysexCommandSysexNonRealtime) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	return nil
}
...
func (m SysexCommandSysexNonRealtime) Box(name string, width int) utils.AsciiBox {
	boxName := "SysexCommandSysexNonRealtime"
	if name != "" {
		boxName += "/" + name
	}
