...
func SysexCommandExtendedAnalogParse(io utils.ReadBuffer) (*SysexCommand, error) {
	if pullErr := io.PullContext("SysexCommandExtendedAnalog"); pullErr != nil {
		return nil, pullErr
	}

	if closeErr := io.CloseContext("SysexCommandExtendedAnalog"); closeErr != nil {
		return nil, closeErr
	}
...
func (m *SysexCommandExtendedAnalog) Serialize(io utils.WriteBuffer) error {
	ser := func() error {
		if pushErr := io.PushContext("SysexCommandExtendedAnalog"); pushErr != nil {
			return pushErr
		}

		if popErr := io.PopContext("SysexCommandExtendedAnalog"); popErr != nil {
			return popErr
		}
		return nil
	}
...
func (m *SysexCommandExtendedAnalog) UnmarshalXML(d *xml.Decoder, start xml.StartElement) error {
	var token xml.Token
	var err error
	foundContent := false
	token = start
...
func (m *SysexCommandExtendedAnalog) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	return nil
}
...
func (m SysexCommandExtendedAnalog) Box(name string, width int) utils.AsciiBox {
	boxName := "SysexCommandExtendedAnalog"
	if name != "" {
		boxName += "/" + name
	}
