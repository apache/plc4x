...
func SysexCommandCapabilityQueryParse(io utils.ReadBuffer) (*SysexCommand, error) {
	if pullErr := io.PullContext("SysexCommandCapabilityQuery"); pullErr != nil {
		return nil, pullErr
	}

	if closeErr := io.CloseContext("SysexCommandCapabilityQuery"); closeErr != nil {
		return nil, closeErr
	}
...
func (m *SysexCommandCapabilityQuery) Serialize(io utils.WriteBuffer) error {
	ser := func() error {
		if pushErr := io.PushContext("SysexCommandCapabilityQuery"); pushErr != nil {
			return pushErr
		}

		if popErr := io.PopContext("SysexCommandCapabilityQuery"); popErr != nil {
			return popErr
		}
		return nil
	}
...
func (m *SysexCommandCapabilityQuery) UnmarshalXML(d *xml.Decoder, start xml.StartElement) error {
	var token xml.Token
	var err error
	foundContent := false
	token = start
...
func (m *SysexCommandCapabilityQuery) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	return nil
}
...
func (m SysexCommandCapabilityQuery) Box(name string, width int) utils.AsciiBox {
	boxName := "SysexCommandCapabilityQuery"
	if name != "" {
		boxName += "/" + name
	}
