...
func SysexCommandCapabilityResponseParse(io utils.ReadBuffer) (*SysexCommand, error) {
	if pullErr := io.PullContext("SysexCommandCapabilityResponse"); pullErr != nil {
		return nil, pullErr
	}

	if closeErr := io.CloseContext("SysexCommandCapabilityResponse"); closeErr != nil {
		return nil, closeErr
	}
...
func (m *SysexCommandCapabilityResponse) Serialize(io utils.WriteBuffer) error {
	ser := func() error {
		if pushErr := io.PushContext("SysexCommandCapabilityResponse"); pushErr != nil {
			return pushErr
		}

		if popErr := io.PopContext("SysexCommandCapabilityResponse"); popErr != nil {
			return popErr
		}
		return nil
	}
...
func (m *SysexCommandCapabilityResponse) UnmarshalXML(d *xml.Decoder, start xml.StartElement) error {
	var token xml.Token
	var err error
	foundContent := false
	token = start
...
func (m *SysexCommandCapabilityResponse) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	return nil
}
...
func (m SysexCommandCapabilityResponse) Box(name string, width int) utils.AsciiBox {
	boxName := "SysexCommandCapabilityResponse"
	if name != "" {
		boxName += "/" + name
	}
