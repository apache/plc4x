...
func SysexCommandSysexRealtimeParse(io utils.ReadBuffer) (*SysexCommand, error) {
	if pullErr := io.PullContext("SysexCommandSysexRealtime"); pullErr != nil {
		return nil, pullErr
	}

	if closeErr := io.CloseContext("SysexCommandSysexRealtime"); closeErr != nil {
		return nil, closeErr
	}
...
func (m *SysexCommandSysexRealtime) Serialize(io utils.WriteBuffer) error {
	ser := func() error {
		if pushErr := io.PushContext("SysexCommandSysexRealtime"); pushErr != nil {
			return pushErr
		}

		if popErr := io.PopContext("SysexCommandSysexRealtime"); popErr != nil {
			return popErr
		}
		return nil
	}
...
func (m *SysexCommandSysexRealtime) UnmarshalXML(d *xml.Decoder, start xml.StartElement) error {
	var token xml.Token
	var err error
	foundContent := false
	token = start
...
func (m *SysexCommandSysexRealtime) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	return nil
}
...
func (m SysexCommandSysexRealtime) Box(name string, width int) utils.AsciiBox {
	boxName := "SysexCommandSysexRealtime"
	if name != "" {
		boxName += "/" + name
	}
