...
func SysexCommandReportFirmwareRequestParse(io utils.ReadBuffer) (*SysexCommand, error) {
	if pullErr := io.PullContext("SysexCommandReportFirmwareRequest"); pullErr != nil {
		return nil, pullErr
	}

	if closeErr := io.CloseContext("SysexCommandReportFirmwareRequest"); closeErr != nil {
		return nil, closeErr
	}
...
func (m *SysexCommandReportFirmwareRequest) Serialize(io utils.WriteBuffer) error {
	ser := func() error {
		if pushErr := io.PushContext("SysexCommandReportFirmwareRequest"); pushErr != nil {
			return pushErr
		}

		if popErr := io.PopContext("SysexCommandReportFirmwareRequest"); popErr != nil {
			return popErr
		}
		return nil
	}
...
func (m *SysexCommandReportFirmwareRequest) UnmarshalXML(d *xml.Decoder, start xml.StartElement) error {
	var token xml.Token
	var err error
	foundContent := false
	token = start
...
func (m *SysexCommandReportFirmwareRequest) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	return nil
}
...
func (m SysexCommandReportFirmwareRequest) Box(name string, width int) utils.AsciiBox {
	boxName := "SysexCommandReportFirmwareRequest"
	if name != "" {
		boxName += "/" + name
	}
