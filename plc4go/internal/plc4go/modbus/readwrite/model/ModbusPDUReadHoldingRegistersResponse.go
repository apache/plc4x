...
func ModbusPDUReadHoldingRegistersResponseParse(io utils.ReadBuffer) (*ModbusPDU, error) {
	if pullErr := io.PullContext("ModbusPDUReadHoldingRegistersResponse"); pullErr != nil {
		return nil, pullErr
	}
	...
	if closeErr := io.CloseContext("ModbusPDUReadHoldingRegistersResponse"); closeErr != nil {
		return nil, closeErr
	}
...
func (m *ModbusPDUReadHoldingRegistersResponse) Serialize(io utils.WriteBuffer) error {
	ser := func() error {
		if pushErr := io.PushContext("ModbusPDUReadHoldingRegistersResponse"); pushErr != nil {
			return pushErr
		}
		...
		if popErr := io.PopContext("ModbusPDUReadHoldingRegistersResponse"); popErr != nil {
			return popErr
		}
		return nil
	}
...
func (m ModbusPDUReadHoldingRegistersResponse) Box(name string, width int) utils.AsciiBox {
	boxName := "ModbusPDUReadHoldingRegistersResponse"
	if name != "" {
		boxName += "/" + name
	}
