...
func BACnetTagContextParse(readBuffer utils.ReadBuffer, tagNumber uint8, extTagNumber uint8, lengthValueType uint8, extLength uint8) (*BACnetTag, error) {
	...
	if closeErr := readBuffer.CloseContext("BACnetTagContext"); closeErr != nil {
		return nil, closeErr
	}
...
func (m *BACnetTagContext) Serialize(writeBuffer utils.WriteBuffer) error {
	ser := func() error {
		if pushErr := writeBuffer.PushContext("BACnetTagContext"); pushErr != nil {
			return pushErr
		}
		...
		if popErr := writeBuffer.PopContext("BACnetTagContext"); popErr != nil {
			return popErr
		}
		return nil
	}
