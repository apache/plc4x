...
func KnxPropertyParse(io utils.ReadBuffer, propertyType KnxPropertyDataType, dataLengthInBytes uint8) (api.PlcValue, error) {
	...
	case propertyType == KnxPropertyDataType_PDT_POLL_GROUP_SETTINGS: // Struct
		...
		for i := 0; i < int((2)); i++ {
			_item, _itemErr := io.ReadUint8("groupAddress", 8)
			if _itemErr != nil {
				return nil, errors.Wrap(_itemErr, "Error parsing 'value' field")
			}
		...
		disable, _disableErr := io.ReadBit("disable")
		if _disableErr != nil {
		...
		pollingSoftNr, _pollingSoftNrErr := io.ReadUint8("pollingSoftNr", 4)
