...
func EipDisconnectRequestParse(io utils.ReadBuffer) (*EipPacket, error) {
	if pullErr := io.PullContext("EipDisconnectRequest"); pullErr != nil {
		return nil, pullErr
	}

	if closeErr := io.CloseContext("EipDisconnectRequest"); closeErr != nil {
		return nil, closeErr
	}
...
func (m *EipDisconnectRequest) Serialize(io utils.WriteBuffer) error {
	ser := func() error {
		if pushErr := io.PushContext("EipDisconnectRequest"); pushErr != nil {
			return pushErr
		}

		if popErr := io.PopContext("EipDisconnectRequest"); popErr != nil {
			return popErr
		}
		return nil
	}
...
func (m *EipDisconnectRequest) UnmarshalXML(d *xml.Decoder, start xml.StartElement) error {
	var token xml.Token
	var err error
	foundContent := false
	token = start
...
func (m *EipDisconnectRequest) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	return nil
}
...
func (m EipDisconnectRequest) Box(name string, width int) utils.AsciiBox {
	boxName := "EipDisconnectRequest"
	if name != "" {
		boxName += "/" + name
	}
