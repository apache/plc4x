...
// the test. The real codec is exercised against the test transport in MessageCodec_test.go; here it
// would only get in the way, as driving a device means driving both ends of the same transport at
// once.
...
// answer hands a response to the pending request, insisting that the request's own accepts predicate
// really considers it an answer - which is what makes these tests catch a broken correlation.
func (c *stubCodec) answer(t *testing.T, request stubRequest, response spi.Message) {
	t.Helper()
	require.True(t, request.acceptsMessage(response), "the request doesn't accept %T as its answer", response)
