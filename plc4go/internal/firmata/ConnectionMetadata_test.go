...
func TestConnection_MetadataMatchesBuilders(t *testing.T) {
	...
	// Firmata has no read path at all: the board pushes the pins it was told to report on and
	// answers no request for a current value. plc4j's FirmataDriver says the same by implementing
	// canWrite and canSubscribe and nothing else.
	assert.False(t, metadata.CanRead(), "firmata has no read path")
	assert.True(t, metadata.CanWrite())
	assert.True(t, metadata.CanSubscribe())
	assert.False(t, metadata.CanBrowse(), "the firmata driver has no browser")
