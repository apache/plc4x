...
const (
	...
	defaultTransportCode = "serial"
	// defaultTcpPort is the port StandardFirmataWiFi.ino and StandardFirmataEthernet.ino listen
	// on, mirroring plc4j's FirmataTcpTransportConfiguration.getDefaultPort.
	defaultTcpPort = "3030"
)

// supportedTransportCodes are the transports this driver speaks, mirroring plc4j
// FirmataDriver.getSupportedTransportCodes:
//
...
//
// The byte stream is identical on all three - what differs is only how the bytes get to the board.
var supportedTransportCodes = []string{"serial", "tcp", "test"}
...
func NewDriver(_options ...options.WithOption) *Driver {
	...
	driver.DefaultDriver = _default.NewDefaultDriver(driver, "firmata", "Firmata", defaultTransportCode, NewTagHandler(_options...))
	return driver
}

// GetSupportedTransportCodes is the list of transports a firmata connection string may name.
...
func (d *Driver) GetConnection(ctx context.Context, transportUrl url.URL, transports map[string]transports.Transport, driverOptions map[string][]string) (plc4go.PlcConnection, error) {
	...
	// Firmata speaks a byte stream, which rules out the datagram transports; refusing them here
	// rather than at the first read keeps a mistyped connection string from looking like a board
	// which never answers.
	if !slices.Contains(supportedTransportCodes, transportUrl.Scheme) {
		connectionLog.Error().
			...
			Msg("The firmata driver doesn't support this transport")
		return nil, errors.Errorf("the firmata driver doesn't support the transport %s, it supports %v", transportUrl.Scheme, supportedTransportCodes)
	}
	...
	// connection string.
	driverOptions["defaultTcpPort"] = []string{defaultTcpPort}
