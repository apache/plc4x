...
// SubscriptionEvent is what a subscriber hands to its consumers. The only thing it adds to the
// default event is the address, which for firmata is simply the pin the value came from.
...
// GetAddress is the tag address the value came from, which for firmata is the pin (or run of pins)
// itself.
