...
const (
	...
	// plc4j UmasDriver.getDefaultTransportCode.
	defaultTransportCode = "tcp"
	// maxConcurrentRequests is what plc4j's UmasConnection.getMaxConcurrentRequests returns: the PLC
	// answers one request at a time.
	maxConcurrentRequests = 1
)

// UmasPort is the TCP port a UMAS PLC listens on - the Modbus port, since UMAS is tunneled inside
// Modbus/TCP. Taken from the mspec constant plc4j's UmasDriver.defaultPorts also uses.
var UmasPort = int(readWriteModel.Constant_UMASTCPDEFAULTPORT)

// supportedTransportCodes are the transports this driver speaks, mirroring plc4j
// UmasDriver.getSupportedTransportCodes:
//
//	tcp:  the real thing
...
func NewDriver(_options ...options.WithOption) plc4go.PlcDriver {
	...
	driver.DefaultDriver = _default.NewDefaultDriver(driver, "umas", "UMAS (Schneider Electric)", defaultTransportCode, NewTagHandler())
	return driver
}

// GetSupportedTransportCodes is the list of transports a umas connection string may name.
...
func (d *Driver) GetConnection(ctx context.Context, transportUrl url.URL, transports map[string]transports.Transport, driverOptions map[string][]string) (plc4go.PlcConnection, error) {
	...
	// Modbus/TCP is a stream protocol, which rules out the datagram transports; refusing them here
	// rather than at the first read keeps a mistyped connection string from looking like a PLC which
	// never answers.
	if !slices.Contains(supportedTransportCodes, transportUrl.Scheme) {
		connectionLog.Error().
			...
			Msg("The umas driver doesn't support this transport")
		return nil, errors.Errorf("the umas driver doesn't support the transport %s, it supports %v", transportUrl.Scheme, supportedTransportCodes)
	}
	...
	// connection string.
	driverOptions["defaultTcpPort"] = []string{strconv.Itoa(UmasPort)}
