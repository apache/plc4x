...
func DF1SymbolMessageFrameParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer) (DF1SymbolMessageFrame, error) {
	...
	messageEnd, err := readConstField[uint8](ctx, "messageEnd", ReadUnsignedByte(readBuffer, 8), DF1SymbolMessageFrame.MESSAGEEND, codegen.WithByteOrder(binary.BigEndian))
	if err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("Error parsing 'messageEnd' field"))
	}
	_ = messageEnd

	endTransaction, err := readConstField[uint8](ctx, "endTransaction", ReadUnsignedByte(readBuffer, 8), DF1SymbolMessageFrame.ENDTRANSACTION, codegen.WithByteOrder(binary.BigEndian))
	if err != nil {
...
func (m *_DF1SymbolMessageFrame) SerializeWithWriteBuffer(ctx context.Context, writeBuffer utils.WriteBuffer) error {
	...
	ser := func() error {
		...
		if err := WriteChecksumField(ctx, "crc", CrcCheck(ctx, m.GetDestinationAddress(), m.GetSourceAddress(), m.GetCommand()), WriteUnsignedInt(writeBuffer, 16), codegen.WithByteOrder(binary.BigEndian)); err != nil {
