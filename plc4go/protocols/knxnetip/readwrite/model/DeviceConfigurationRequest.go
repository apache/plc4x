...
func (m *_DeviceConfigurationRequestBuilder) WithCemiBuilder(builderSupplier func(CEMIBuilder) CEMIBuilder) DeviceConfigurationRequestBuilder {
	builder := builderSupplier(m.Cemi.CreateCEMIBuilder())
	var err error
	m.Cemi, err = builder.Build()
	if err != nil {
		if m.err == nil {
			m.err = &utils.MultiError{MainError: errors.New("sub builder failed")}
		}
		m.err.Append(errors.Wrap(err, "CEMIBuilder failed"))
	}
	return m
}
