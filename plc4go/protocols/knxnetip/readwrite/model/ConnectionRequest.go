...
func (m *_ConnectionRequestBuilder) WithConnectionRequestInformationBuilder(builderSupplier func(ConnectionRequestInformationBuilder) ConnectionRequestInformationBuilder) ConnectionRequestBuilder {
	builder := builderSupplier(m.ConnectionRequestInformation.CreateConnectionRequestInformationBuilder())
	var err error
	m.ConnectionRequestInformation, err = builder.Build()
	if err != nil {
		if m.err == nil {
			m.err = &utils.MultiError{MainError: errors.New("sub builder failed")}
		}
		m.err.Append(errors.Wrap(err, "ConnectionRequestInformationBuilder failed"))
	}
	return m
}
