...
func (m *_ApduDataExt) parse(ctx context.Context, readBuffer utils.ReadBuffer, length uint8) (__apduDataExt ApduDataExt, err error) {
	...
	case extApciType == 0x00: // ApduDataExtOpenRoutingTableRequest
		if _child, err = new(_ApduDataExtOpenRoutingTableRequest).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x01: // ApduDataExtReadRoutingTableRequest
		if _child, err = new(_ApduDataExtReadRoutingTableRequest).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x02: // ApduDataExtReadRoutingTableResponse
		if _child, err = new(_ApduDataExtReadRoutingTableResponse).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x03: // ApduDataExtWriteRoutingTableRequest
		if _child, err = new(_ApduDataExtWriteRoutingTableRequest).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x08: // ApduDataExtReadRouterMemoryRequest
		if _child, err = new(_ApduDataExtReadRouterMemoryRequest).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x09: // ApduDataExtReadRouterMemoryResponse
		if _child, err = new(_ApduDataExtReadRouterMemoryResponse).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x0A: // ApduDataExtWriteRouterMemoryRequest
		if _child, err = new(_ApduDataExtWriteRouterMemoryRequest).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x0D: // ApduDataExtReadRouterStatusRequest
		if _child, err = new(_ApduDataExtReadRouterStatusRequest).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x0E: // ApduDataExtReadRouterStatusResponse
		if _child, err = new(_ApduDataExtReadRouterStatusResponse).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x0F: // ApduDataExtWriteRouterStatusRequest
		if _child, err = new(_ApduDataExtWriteRouterStatusRequest).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x10: // ApduDataExtMemoryBitWrite
		if _child, err = new(_ApduDataExtMemoryBitWrite).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x11: // ApduDataExtAuthorizeRequest
		if _child, err = new(_ApduDataExtAuthorizeRequest).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x12: // ApduDataExtAuthorizeResponse
		if _child, err = new(_ApduDataExtAuthorizeResponse).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x13: // ApduDataExtKeyWrite
		if _child, err = new(_ApduDataExtKeyWrite).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x14: // ApduDataExtKeyResponse
		if _child, err = new(_ApduDataExtKeyResponse).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x15: // ApduDataExtPropertyValueRead
		if _child, err = new(_ApduDataExtPropertyValueRead).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x16: // ApduDataExtPropertyValueResponse
		if _child, err = new(_ApduDataExtPropertyValueResponse).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x17: // ApduDataExtPropertyValueWrite
		if _child, err = new(_ApduDataExtPropertyValueWrite).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x18: // ApduDataExtPropertyDescriptionRead
		if _child, err = new(_ApduDataExtPropertyDescriptionRead).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x19: // ApduDataExtPropertyDescriptionResponse
		if _child, err = new(_ApduDataExtPropertyDescriptionResponse).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x1A: // ApduDataExtNetworkParameterRead
		if _child, err = new(_ApduDataExtNetworkParameterRead).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x1B: // ApduDataExtNetworkParameterResponse
		if _child, err = new(_ApduDataExtNetworkParameterResponse).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x1C: // ApduDataExtIndividualAddressSerialNumberRead
		if _child, err = new(_ApduDataExtIndividualAddressSerialNumberRead).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x1D: // ApduDataExtIndividualAddressSerialNumberResponse
		if _child, err = new(_ApduDataExtIndividualAddressSerialNumberResponse).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x1E: // ApduDataExtIndividualAddressSerialNumberWrite
		if _child, err = new(_ApduDataExtIndividualAddressSerialNumberWrite).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x20: // ApduDataExtDomainAddressWrite
		if _child, err = new(_ApduDataExtDomainAddressWrite).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x21: // ApduDataExtDomainAddressRead
		if _child, err = new(_ApduDataExtDomainAddressRead).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x22: // ApduDataExtDomainAddressResponse
		if _child, err = new(_ApduDataExtDomainAddressResponse).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x23: // ApduDataExtDomainAddressSelectiveRead
		if _child, err = new(_ApduDataExtDomainAddressSelectiveRead).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x24: // ApduDataExtNetworkParameterWrite
		if _child, err = new(_ApduDataExtNetworkParameterWrite).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x25: // ApduDataExtLinkRead
		if _child, err = new(_ApduDataExtLinkRead).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x26: // ApduDataExtLinkResponse
		if _child, err = new(_ApduDataExtLinkResponse).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x27: // ApduDataExtLinkWrite
		if _child, err = new(_ApduDataExtLinkWrite).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x28: // ApduDataExtGroupPropertyValueRead
		if _child, err = new(_ApduDataExtGroupPropertyValueRead).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x29: // ApduDataExtGroupPropertyValueResponse
		if _child, err = new(_ApduDataExtGroupPropertyValueResponse).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x2A: // ApduDataExtGroupPropertyValueWrite
		if _child, err = new(_ApduDataExtGroupPropertyValueWrite).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x2B: // ApduDataExtGroupPropertyValueInfoReport
		if _child, err = new(_ApduDataExtGroupPropertyValueInfoReport).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x2C: // ApduDataExtDomainAddressSerialNumberRead
		if _child, err = new(_ApduDataExtDomainAddressSerialNumberRead).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x2D: // ApduDataExtDomainAddressSerialNumberResponse
		if _child, err = new(_ApduDataExtDomainAddressSerialNumberResponse).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x2E: // ApduDataExtDomainAddressSerialNumberWrite
		if _child, err = new(_ApduDataExtDomainAddressSerialNumberWrite).parse(ctx, readBuffer, m, uint8(length)); err != nil {
	...
	case extApciType == 0x30: // ApduDataExtFileStreamInfoReport
		if _child, err = new(_ApduDataExtFileStreamInfoReport).parse(ctx, readBuffer, m, uint8(length)); err != nil {
