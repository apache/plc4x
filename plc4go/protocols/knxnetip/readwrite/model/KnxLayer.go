...
func KnxLayerParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer) (KnxLayer, error) {
	log := zerolog.Ctx(ctx)
	_ = log
	val, err := /*TODO: migrate me*/ /*TODO: migrate me*/ readBuffer.ReadUint8("KnxLayer", 8)
	if err != nil {
		return 0, errors.Wrap(err, "error reading KnxLayer")
	}
	if enum, ok := KnxLayerByValue(val); !ok {
		log.Debug().Interface("val", val).Msg("no value val found for KnxLayer")
...
func (e KnxLayer) SerializeWithWriteBuffer(ctx context.Context, writeBuffer utils.WriteBuffer) error {
	log := zerolog.Ctx(ctx)
	_ = log
	return /*TODO: migrate me*/ writeBuffer.WriteUint8("KnxLayer", 8, uint8(uint8(e)), utils.WithAdditionalStringRepresentation(e.PLC4XEnumName()))
}
