...
func (m *_ConnectionResponseBuilder) WithOptionalConnectionResponseDataBlockBuilder(builderSupplier func(ConnectionResponseDataBlockBuilder) ConnectionResponseDataBlockBuilder) ConnectionResponseBuilder {
	builder := builderSupplier(m.ConnectionResponseDataBlock.CreateConnectionResponseDataBlockBuilder())
	var err error
	m.ConnectionResponseDataBlock, err = builder.Build()
	if err != nil {
		if m.err == nil {
			m.err = &utils.MultiError{MainError: errors.New("sub builder failed")}
		}
		m.err.Append(errors.Wrap(err, "ConnectionResponseDataBlockBuilder failed"))
	}
	return m
}
