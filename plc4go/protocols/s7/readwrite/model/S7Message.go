...
func (m *_S7MessageBuilder) WithOptionalParameterBuilder(builderSupplier func(S7ParameterBuilder) S7ParameterBuilder) S7MessageBuilder {
	builder := builderSupplier(m.Parameter.CreateS7ParameterBuilder())
	var err error
	m.Parameter, err = builder.Build()
	if err != nil {
		if m.err == nil {
			m.err = &utils.MultiError{MainError: errors.New("sub builder failed")}
		}
		m.err.Append(errors.Wrap(err, "S7ParameterBuilder failed"))
	}
	return m
}
...
func (m *_S7MessageBuilder) WithOptionalPayloadBuilder(builderSupplier func(S7PayloadBuilder) S7PayloadBuilder) S7MessageBuilder {
	builder := builderSupplier(m.Payload.CreateS7PayloadBuilder())
	var err error
	...
	if err != nil {
		...
		m.err.Append(errors.Wrap(err, "S7PayloadBuilder failed"))
	}
	return m
}
