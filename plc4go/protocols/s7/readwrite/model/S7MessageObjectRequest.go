...
func S7MessageObjectRequestParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, cpuFunctionType uint8) (S7MessageObjectRequest, error) {
	...
	variableSpec, err := readConstField[uint8](ctx, "variableSpec", ReadUnsignedByte(readBuffer, 8), S7MessageObjectRequest.VARIABLESPEC)
	if err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("Error parsing 'variableSpec' field"))
	}
	_ = variableSpec

	length, err := readConstField[uint8](ctx, "length", ReadUnsignedByte(readBuffer, 8), S7MessageObjectRequest.LENGTH)
	if err != nil {
