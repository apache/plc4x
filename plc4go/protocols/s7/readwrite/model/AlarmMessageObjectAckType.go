...
func AlarmMessageObjectAckTypeParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer) (AlarmMessageObjectAckType, error) {
	...
	variableSpec, err := readConstField[uint8](ctx, "variableSpec", ReadUnsignedByte(readBuffer, 8), AlarmMessageObjectAckType.VARIABLESPEC)
	if err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("Error parsing 'variableSpec' field"))
	}
	_ = variableSpec

	length, err := readConstField[uint8](ctx, "length", ReadUnsignedByte(readBuffer, 8), AlarmMessageObjectAckType.LENGTH)
	if err != nil {
