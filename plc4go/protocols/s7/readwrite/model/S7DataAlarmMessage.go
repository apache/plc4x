...
func S7DataAlarmMessageParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, cpuFunctionType uint8) (S7DataAlarmMessage, error) {
	...
	functionId, err := readConstField[uint8](ctx, "functionId", ReadUnsignedByte(readBuffer, 8), S7DataAlarmMessage.FUNCTIONID)
	if err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("Error parsing 'functionId' field"))
	}
	_ = functionId

	numberMessageObj, err := readConstField[uint8](ctx, "numberMessageObj", ReadUnsignedByte(readBuffer, 8), S7DataAlarmMessage.NUMBERMESSAGEOBJ)
	if err != nil {
