...
func (m DataTransportErrorCode) GetLengthInBits(ctx context.Context) uint64 {
	return 8
}

func (m DataTransportErrorCode) GetLengthInBytes(ctx context.Context) uint64 {
...
func DataTransportErrorCodeParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer) (DataTransportErrorCode, error) {
	log := zerolog.Ctx(ctx)
	_ = log
	val, err := /*TODO: migrate me*/ /*TODO: migrate me*/ readBuffer.ReadUint8("DataTransportErrorCode", 8)
	if err != nil {
		return 0, errors.Wrap(err, "error reading DataTransportErrorCode")
	}
	if enum, ok := DataTransportErrorCodeByValue(val); !ok {
		log.Debug().Interface("val", val).Msg("no value val found for DataTransportErrorCode")
