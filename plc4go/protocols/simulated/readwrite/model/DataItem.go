...
func DataItemParse(theBytes []byte, dataType string, numberOfValues uint16) (api.PlcValue, error) {
	return DataItemParseWithBuffer(utils.NewReadBufferByteBased(theBytes, utils.WithByteOrderForReadBufferByteBased(binary.BigEndian)), dataType, numberOfValues) // TODO: get endianness from mspec
}

func DataItemParseWithBuffer(readBuffer utils.ReadBuffer, dataType string, numberOfValues uint16) (api.PlcValue, error) {
...
func DataItemSerializeWithWriteBuffer(writeBuffer utils.WriteBuffer, value api.PlcValue, dataType string, numberOfValues uint16) error {
	m := struct {
		DataType       string
