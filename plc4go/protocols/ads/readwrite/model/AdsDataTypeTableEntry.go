...
func AdsDataTypeTableEntryParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer) (AdsDataTypeTableEntry, error) {
	...
	dataTypeNameTerminator, err := readConstField[uint8](ctx, "dataTypeNameTerminator", ReadUnsignedByte(readBuffer, 8), AdsDataTypeTableEntry.DATATYPENAMETERMINATOR, codegen.WithByteOrder(binary.LittleEndian))
	if err != nil {
	...
	simpleTypeNameTerminator, err := readConstField[uint8](ctx, "simpleTypeNameTerminator", ReadUnsignedByte(readBuffer, 8), AdsDataTypeTableEntry.SIMPLETYPENAMETERMINATOR, codegen.WithByteOrder(binary.LittleEndian))
	if err != nil {
	...
	commentTerminator, err := readConstField[uint8](ctx, "commentTerminator", ReadUnsignedByte(readBuffer, 8), AdsDataTypeTableEntry.COMMENTTERMINATOR, codegen.WithByteOrder(binary.LittleEndian))
	if err != nil {
