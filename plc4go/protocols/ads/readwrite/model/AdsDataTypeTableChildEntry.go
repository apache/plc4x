...
func AdsDataTypeTableChildEntryParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer) (AdsDataTypeTableChildEntry, error) {
	...
	propertyNameTerminator, err := readConstField[uint8](ctx, "propertyNameTerminator", ReadUnsignedByte(readBuffer, 8), AdsDataTypeTableChildEntry.PROPERTYNAMETERMINATOR, codegen.WithByteOrder(binary.LittleEndian))
	if err != nil {
	...
	dataTypeNameTerminator, err := readConstField[uint8](ctx, "dataTypeNameTerminator", ReadUnsignedByte(readBuffer, 8), AdsDataTypeTableChildEntry.DATATYPENAMETERMINATOR, codegen.WithByteOrder(binary.LittleEndian))
	if err != nil {
	...
	commentTerminator, err := readConstField[uint8](ctx, "commentTerminator", ReadUnsignedByte(readBuffer, 8), AdsDataTypeTableChildEntry.COMMENTTERMINATOR, codegen.WithByteOrder(binary.LittleEndian))
	if err != nil {
