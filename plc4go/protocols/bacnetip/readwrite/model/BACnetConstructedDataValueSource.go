...
func (m *_BACnetConstructedDataValueSourceBuilder) WithValueSourceBuilder(builderSupplier func(BACnetValueSourceBuilder) BACnetValueSourceBuilder) BACnetConstructedDataValueSourceBuilder {
	builder := builderSupplier(m.ValueSource.CreateBACnetValueSourceBuilder())
	var err error
	m.ValueSource, err = builder.Build()
	if err != nil {
		if m.err == nil {
			m.err = &utils.MultiError{MainError: errors.New("sub builder failed")}
		}
		m.err.Append(errors.Wrap(err, "BACnetValueSourceBuilder failed"))
	}
	return m
}
