...
func BACnetNetworkPortCommandTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetNetworkPortCommandTagged, error) {
	...
	value, err := ReadManualField[BACnetNetworkPortCommand](ctx, "value", readBuffer, func(ctx context.Context) (BACnetNetworkPortCommand, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetNetworkPortCommand_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetNetworkPortCommand
		if err != nil {
			return zero, err
		}
		return v.(BACnetNetworkPortCommand), err
	})
	if err != nil {
