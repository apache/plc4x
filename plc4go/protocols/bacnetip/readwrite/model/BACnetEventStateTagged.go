...
func BACnetEventStateTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetEventStateTagged, error) {
	...
	value, err := ReadManualField[BACnetEventState](ctx, "value", readBuffer, func(ctx context.Context) (BACnetEventState, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetEventState_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetEventState
		if err != nil {
			return zero, err
		}
		return v.(BACnetEventState), err
	})
	if err != nil {
