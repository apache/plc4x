...
func (m *_BACnetConstructedDataFaultParametersBuilder) WithFaultParametersBuilder(builderSupplier func(BACnetFaultParameterBuilder) BACnetFaultParameterBuilder) BACnetConstructedDataFaultParametersBuilder {
	builder := builderSupplier(m.FaultParameters.CreateBACnetFaultParameterBuilder())
	var err error
	m.FaultParameters, err = builder.Build()
	if err != nil {
		if m.err == nil {
			m.err = &utils.MultiError{MainError: errors.New("sub builder failed")}
		}
		m.err.Append(errors.Wrap(err, "BACnetFaultParameterBuilder failed"))
	}
	return m
}
