...
func BACnetAuthorizationModeTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetAuthorizationModeTagged, error) {
	...
	value, err := ReadManualField[BACnetAuthorizationMode](ctx, "value", readBuffer, func(ctx context.Context) (BACnetAuthorizationMode, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetAuthorizationMode_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetAuthorizationMode
		if err != nil {
			return zero, err
		}
		return v.(BACnetAuthorizationMode), err
	})
	if err != nil {
