...
func (m *_BACnetPropertyAccessResultBuilder) WithAccessResultBuilder(builderSupplier func(BACnetPropertyAccessResultAccessResultBuilder) BACnetPropertyAccessResultAccessResultBuilder) BACnetPropertyAccessResultBuilder {
	builder := builderSupplier(m.AccessResult.CreateBACnetPropertyAccessResultAccessResultBuilder())
	var err error
	m.AccessResult, err = builder.Build()
	if err != nil {
		if m.err == nil {
			m.err = &utils.MultiError{MainError: errors.New("sub builder failed")}
		}
		m.err.Append(errors.Wrap(err, "BACnetPropertyAccessResultAccessResultBuilder failed"))
	}
	return m
}
