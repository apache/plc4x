...
func BACnetContextTagUnsignedIntegerParse(theBytes []byte, tagNumberArgument uint8, dataType BACnetDataType, header BACnetTagHeader) (BACnetContextTagUnsignedInteger, error) {
	return BACnetContextTagUnsignedIntegerParseWithBuffer(utils.NewReadBufferByteBased(theBytes, utils.WithByteOrderForReadBufferByteBased(binary.BigEndian)), tagNumberArgument, dataType, header) // TODO: get endianness from mspec
}
...
func (m *_BACnetContextTagUnsignedInteger) SerializeWithWriteBuffer(writeBuffer utils.WriteBuffer) error {
