...
func BACnetRelationshipTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetRelationshipTagged, error) {
	...
	value, err := ReadManualField[BACnetRelationship](ctx, "value", readBuffer, func(ctx context.Context) (BACnetRelationship, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetRelationship_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetRelationship
		if err != nil {
			return zero, err
		}
		return v.(BACnetRelationship), err
	})
	if err != nil {
