...
func BACnetLifeSafetyOperationTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetLifeSafetyOperationTagged, error) {
	...
	value, err := ReadManualField[BACnetLifeSafetyOperation](ctx, "value", readBuffer, func(ctx context.Context) (BACnetLifeSafetyOperation, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetLifeSafetyOperation_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetLifeSafetyOperation
		if err != nil {
			return zero, err
		}
		return v.(BACnetLifeSafetyOperation), err
	})
	if err != nil {
