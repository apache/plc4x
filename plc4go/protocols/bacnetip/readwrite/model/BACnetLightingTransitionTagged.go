...
func BACnetLightingTransitionTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetLightingTransitionTagged, error) {
	...
	value, err := ReadManualField[BACnetLightingTransition](ctx, "value", readBuffer, func(ctx context.Context) (BACnetLightingTransition, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetLightingTransition_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetLightingTransition
		if err != nil {
			return zero, err
		}
		return v.(BACnetLightingTransition), err
	})
	if err != nil {
