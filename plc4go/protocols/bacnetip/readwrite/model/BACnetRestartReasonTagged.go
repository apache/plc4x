...
func BACnetRestartReasonTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetRestartReasonTagged, error) {
	...
	value, err := ReadManualField[BACnetRestartReason](ctx, "value", readBuffer, func(ctx context.Context) (BACnetRestartReason, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetRestartReason_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetRestartReason
		if err != nil {
			return zero, err
		}
		return v.(BACnetRestartReason), err
	})
	if err != nil {
