...
func BACnetDeviceStatusTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetDeviceStatusTagged, error) {
	...
	value, err := ReadManualField[BACnetDeviceStatus](ctx, "value", readBuffer, func(ctx context.Context) (BACnetDeviceStatus, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetDeviceStatus_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetDeviceStatus
		if err != nil {
			return zero, err
		}
		return v.(BACnetDeviceStatus), err
	})
	if err != nil {
