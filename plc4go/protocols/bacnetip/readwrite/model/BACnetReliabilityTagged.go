...
func BACnetReliabilityTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetReliabilityTagged, error) {
	...
	value, err := ReadManualField[BACnetReliability](ctx, "value", readBuffer, func(ctx context.Context) (BACnetReliability, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetReliability_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetReliability
		if err != nil {
			return zero, err
		}
		return v.(BACnetReliability), err
	})
	if err != nil {
