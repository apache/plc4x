...
func BACnetLifeSafetyModeTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetLifeSafetyModeTagged, error) {
	...
	value, err := ReadManualField[BACnetLifeSafetyMode](ctx, "value", readBuffer, func(ctx context.Context) (BACnetLifeSafetyMode, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetLifeSafetyMode_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetLifeSafetyMode
		if err != nil {
			return zero, err
		}
		return v.(BACnetLifeSafetyMode), err
	})
	if err != nil {
