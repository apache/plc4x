...
func BACnetAccessCredentialDisableReasonTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetAccessCredentialDisableReasonTagged, error) {
	...
	value, err := ReadManualField[BACnetAccessCredentialDisableReason](ctx, "value", readBuffer, func(ctx context.Context) (BACnetAccessCredentialDisableReason, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetAccessCredentialDisableReason_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetAccessCredentialDisableReason
		if err != nil {
			return zero, err
		}
		return v.(BACnetAccessCredentialDisableReason), err
	})
	if err != nil {
