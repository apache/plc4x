...
func BACnetEventTypeTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetEventTypeTagged, error) {
	...
	value, err := ReadManualField[BACnetEventType](ctx, "value", readBuffer, func(ctx context.Context) (BACnetEventType, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetEventType_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetEventType
		if err != nil {
			return zero, err
		}
		return v.(BACnetEventType), err
	})
	if err != nil {
