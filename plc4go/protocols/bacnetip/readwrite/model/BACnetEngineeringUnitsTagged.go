...
func BACnetEngineeringUnitsTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetEngineeringUnitsTagged, error) {
	...
	value, err := ReadManualField[BACnetEngineeringUnits](ctx, "value", readBuffer, func(ctx context.Context) (BACnetEngineeringUnits, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetEngineeringUnits_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetEngineeringUnits
		if err != nil {
			return zero, err
		}
		return v.(BACnetEngineeringUnits), err
	})
	if err != nil {
