...
func (m *_NPDUBuilder) WithOptionalNlmBuilder(builderSupplier func(NLMBuilder) NLMBuilder) NPDUBuilder {
	builder := builderSupplier(m.Nlm.CreateNLMBuilder())
	var err error
	m.Nlm, err = builder.Build()
	if err != nil {
		if m.err == nil {
			m.err = &utils.MultiError{MainError: errors.New("sub builder failed")}
		}
		m.err.Append(errors.Wrap(err, "NLMBuilder failed"))
	}
	return m
}
...
func (m *_NPDUBuilder) WithOptionalApduBuilder(builderSupplier func(APDUBuilder) APDUBuilder) NPDUBuilder {
	builder := builderSupplier(m.Apdu.CreateAPDUBuilder())
	var err error
	m.Apdu, err = builder.Build()
	if err != nil {
		if m.err == nil {
			m.err = &utils.MultiError{MainError: errors.New("sub builder failed")}
		}
		m.err.Append(errors.Wrap(err, "APDUBuilder failed"))
	}
	return m
}
