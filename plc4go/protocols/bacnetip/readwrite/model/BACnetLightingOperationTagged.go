...
func BACnetLightingOperationTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetLightingOperationTagged, error) {
	...
	value, err := ReadManualField[BACnetLightingOperation](ctx, "value", readBuffer, func(ctx context.Context) (BACnetLightingOperation, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetLightingOperation_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetLightingOperation
		if err != nil {
			return zero, err
		}
		return v.(BACnetLightingOperation), err
	})
	if err != nil {
