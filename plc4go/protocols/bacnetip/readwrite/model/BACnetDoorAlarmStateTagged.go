...
func BACnetDoorAlarmStateTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetDoorAlarmStateTagged, error) {
	...
	value, err := ReadManualField[BACnetDoorAlarmState](ctx, "value", readBuffer, func(ctx context.Context) (BACnetDoorAlarmState, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetDoorAlarmState_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetDoorAlarmState
		if err != nil {
			return zero, err
		}
		return v.(BACnetDoorAlarmState), err
	})
	if err != nil {
