...
func BACnetConstructedDataPositiveAccessRulesParse(ctx context.Context, theBytes []byte, tagNumber uint8, objectTypeArgument BACnetObjectType, propertyIdentifierArgument BACnetPropertyIdentifier, arrayIndexArgument BACnetTagPayloadUnsignedInteger) (BACnetConstructedDataPositiveAccessRules, error) {
	return BACnetConstructedDataPositiveAccessRulesParseWithBuffer(ctx, utils.NewReadBufferByteBased(theBytes), tagNumber, objectTypeArgument, propertyIdentifierArgument, arrayIndexArgument)
}

func BACnetConstructedDataPositiveAccessRulesParseWithBufferProducer(tagNumber uint8, objectTypeArgument BACnetObjectType, propertyIdentifierArgument BACnetPropertyIdentifier, arrayIndexArgument BACnetTagPayloadUnsignedInteger) func(ctx context.Context, readBuffer utils.ReadBuffer) (BACnetConstructedDataPositiveAccessRules, error) {
	return func(ctx context.Context, readBuffer utils.ReadBuffer) (BACnetConstructedDataPositiveAccessRules, error) {
		return BACnetConstructedDataPositiveAccessRulesParseWithBuffer(ctx, readBuffer, tagNumber, objectTypeArgument, propertyIdentifierArgument, arrayIndexArgument)
	}
}
...
func (m *_BACnetConstructedDataPositiveAccessRules) SerializeWithWriteBuffer(ctx context.Context, writeBuffer utils.WriteBuffer) error {
	...
	ser := func() error {
		...
		for _curItem, _element := range m.GetPositiveAccessRules() {
			_ = _curItem
			arrayCtx := utils.CreateArrayContext(ctx, len(m.GetPositiveAccessRules()), _curItem)
			_ = arrayCtx
