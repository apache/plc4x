...
func BACnetLoggingTypeTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetLoggingTypeTagged, error) {
	...
	value, err := ReadManualField[BACnetLoggingType](ctx, "value", readBuffer, func(ctx context.Context) (BACnetLoggingType, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetLoggingType_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetLoggingType
		if err != nil {
			return zero, err
		}
		return v.(BACnetLoggingType), err
	})
	if err != nil {
