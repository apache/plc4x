...
func BACnetDoorStatusTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetDoorStatusTagged, error) {
	...
	value, err := ReadManualField[BACnetDoorStatus](ctx, "value", readBuffer, func(ctx context.Context) (BACnetDoorStatus, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetDoorStatus_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetDoorStatus
		if err != nil {
			return zero, err
		}
		return v.(BACnetDoorStatus), err
	})
	if err != nil {
