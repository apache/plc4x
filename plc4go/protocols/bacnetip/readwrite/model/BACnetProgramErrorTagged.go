...
func BACnetProgramErrorTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetProgramErrorTagged, error) {
	...
	value, err := ReadManualField[BACnetProgramError](ctx, "value", readBuffer, func(ctx context.Context) (BACnetProgramError, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetProgramError_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetProgramError
		if err != nil {
			return zero, err
		}
		return v.(BACnetProgramError), err
	})
	if err != nil {
