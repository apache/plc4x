...
func BACnetSilencedStateTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetSilencedStateTagged, error) {
	...
	value, err := ReadManualField[BACnetSilencedState](ctx, "value", readBuffer, func(ctx context.Context) (BACnetSilencedState, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetSilencedState_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetSilencedState
		if err != nil {
			return zero, err
		}
		return v.(BACnetSilencedState), err
	})
	if err != nil {
