...
func BACnetLiftCarModeTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetLiftCarModeTagged, error) {
	...
	value, err := ReadManualField[BACnetLiftCarMode](ctx, "value", readBuffer, func(ctx context.Context) (BACnetLiftCarMode, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetLiftCarMode_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetLiftCarMode
		if err != nil {
			return zero, err
		}
		return v.(BACnetLiftCarMode), err
	})
	if err != nil {
