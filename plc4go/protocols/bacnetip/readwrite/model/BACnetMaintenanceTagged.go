...
func BACnetMaintenanceTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetMaintenanceTagged, error) {
	...
	value, err := ReadManualField[BACnetMaintenance](ctx, "value", readBuffer, func(ctx context.Context) (BACnetMaintenance, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetMaintenance_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetMaintenance
		if err != nil {
			return zero, err
		}
		return v.(BACnetMaintenance), err
	})
	if err != nil {
