...
func BACnetLifeSafetyStateTaggedParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, tagClass TagClass) (BACnetLifeSafetyStateTagged, error) {
	...
	value, err := ReadManualField[BACnetLifeSafetyState](ctx, "value", readBuffer, func(ctx context.Context) (BACnetLifeSafetyState, error) {
		v, err := ReadEnumGeneric(ctx, readBuffer, header.GetActualLength(), BACnetLifeSafetyState_VENDOR_PROPRIETARY_VALUE)
		var zero BACnetLifeSafetyState
		if err != nil {
			return zero, err
		}
		return v.(BACnetLifeSafetyState), err
	})
	if err != nil {
