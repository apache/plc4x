...
func BACnetConstructedDataStateTextParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, tagNumber uint8, objectTypeArgument BACnetObjectType, propertyIdentifierArgument BACnetPropertyIdentifier, arrayIndexArgument BACnetTagPayloadUnsignedInteger) (BACnetConstructedDataStateText, error) {
	...
	stateText, err := ReadTerminatedArrayField[BACnetApplicationTagCharacterString](ctx, "stateText", ReadComplex[BACnetApplicationTagCharacterString](func(ctx context.Context, buffer utils.ReadBuffer) (BACnetApplicationTagCharacterString, error) {
		v, err := BACnetApplicationTagParseWithBuffer(ctx, readBuffer)
		if err != nil {
			return nil, err
		}
