...
// VariantGuidExactly can be used when we want exactly this type and not a type which fulfills VariantGuid.
// This is useful for switch cases.
type VariantGuidExactly interface {
	VariantGuid
	isVariantGuid() bool
}
...
func (m *_VariantGuid) InitializeParent(parent Variant, arrayLengthSpecified bool, arrayDimensionsSpecified bool, noOfArrayDimensions *int32, arrayDimensions []bool) {
...
func (m *_VariantGuid) GetParent() Variant {
	return m._Variant
}
...
func VariantGuidParse(ctx context.Context, theBytes []byte, arrayLengthSpecified bool) (VariantGuid, error) {
	return VariantGuidParseWithBuffer(ctx, utils.NewReadBufferByteBased(theBytes), arrayLengthSpecified)
}

func VariantGuidParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, arrayLengthSpecified bool) (VariantGuid, error) {
	...
	if arrayLengthSpecified {
		...
		if _err != nil {
			return nil, errors.Wrap(_err, "Error parsing 'arrayLength' field of VariantGuid")
		}
		arrayLength = &_val
	}
	...
	// Count array
	value := make([]GuidValue, max(utils.InlineIf(bool((arrayLength) == (nil)), func() any { return uint16(uint16(1)) }, func() any { return uint16((*arrayLength)) }).(uint16), 0))
	...
	}
	{
		...
		for _curItem := uint16(0); _curItem < _numItems; _curItem++ {
			...
			_ = arrayCtx
			_ = _curItem
			_item, _err := GuidValueParseWithBuffer(arrayCtx, readBuffer)
			if _err != nil {
				return nil, errors.Wrap(_err, "Error parsing 'value' field of VariantGuid")
			}
			value[_curItem] = _item.(GuidValue)
		}
	}
...
func (m *_VariantGuid) isVariantGuid() bool {
	return true
}
