...
func (m *_ModbusAsciiADU) SerializeWithWriteBuffer(ctx context.Context, writeBuffer utils.WriteBuffer) error {
	...
	ser := func() error {
		...
		if err := WriteChecksumField(ctx, "crc", AsciiLrcCheck(ctx, m.GetAddress(), m.GetPdu()), WriteUnsignedShort(writeBuffer, 8), codegen.WithByteOrder(binary.BigEndian)); err != nil {
