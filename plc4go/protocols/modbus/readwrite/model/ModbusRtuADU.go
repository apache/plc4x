...
func ModbusRtuADUParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, driverType DriverType, response bool) (ModbusRtuADU, error) {
	...
	crc, err := ReadChecksumField[uint16](ctx, "crc", ReadUnsignedShort(readBuffer, 16), RtuCrcCheck(ctx, address, pdu), codegen.WithByteOrder(binary.BigEndian))
	if err != nil {
...
func (m *_ModbusRtuADU) SerializeWithWriteBuffer(ctx context.Context, writeBuffer utils.WriteBuffer) error {
	...
	ser := func() error {
		...
		if err := WriteChecksumField(ctx, "crc", RtuCrcCheck(ctx, m.GetAddress(), m.GetPdu()), WriteUnsignedInt(writeBuffer, 16), codegen.WithByteOrder(binary.BigEndian)); err != nil {
