...
func EipConstantsParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer) (EipConstants, error) {
	...
	eipUdpDiscoveryDefaultPort, err := readConstField[uint16](ctx, "eipUdpDiscoveryDefaultPort", ReadUnsignedShort(readBuffer, 16), EipConstants.EIPUDPDISCOVERYDEFAULTPORT)
	if err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("Error parsing 'eipUdpDiscoveryDefaultPort' field"))
	}
	_ = eipUdpDiscoveryDefaultPort

	eipTcpDefaultPort, err := readConstField[uint16](ctx, "eipTcpDefaultPort", ReadUnsignedShort(readBuffer, 16), EipConstants.EIPTCPDEFAULTPORT)
	if err != nil {
