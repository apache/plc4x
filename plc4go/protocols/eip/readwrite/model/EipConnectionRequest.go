...
func EipConnectionRequestParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, response bool) (EipConnectionRequest, error) {
	...
	protocolVersion, err := readConstField[uint16](ctx, "protocolVersion", ReadUnsignedShort(readBuffer, 16), EipConnectionRequest.PROTOCOLVERSION)
	if err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("Error parsing 'protocolVersion' field"))
	}
	_ = protocolVersion

	flags, err := readConstField[uint16](ctx, "flags", ReadUnsignedShort(readBuffer, 16), EipConnectionRequest.FLAGS)
	if err != nil {
