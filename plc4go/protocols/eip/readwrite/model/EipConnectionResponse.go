...
func EipConnectionResponseParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer, response bool) (EipConnectionResponse, error) {
	...
	protocolVersion, err := readConstField[uint16](ctx, "protocolVersion", ReadUnsignedShort(readBuffer, 16), EipConnectionResponse.PROTOCOLVERSION)
	if err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("Error parsing 'protocolVersion' field"))
	}
	_ = protocolVersion

	flags, err := readConstField[uint16](ctx, "flags", ReadUnsignedShort(readBuffer, 16), EipConnectionResponse.FLAGS)
	if err != nil {
