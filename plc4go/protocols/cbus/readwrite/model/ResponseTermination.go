...
func ResponseTerminationParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer) (ResponseTermination, error) {
	...
	cr, err := readConstField[byte](ctx, "cr", ReadByte(readBuffer, 8), ResponseTermination.CR)
	if err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("Error parsing 'cr' field"))
	}
	_ = cr

	lf, err := readConstField[byte](ctx, "lf", ReadByte(readBuffer, 8), ResponseTermination.LF)
	if err != nil {
