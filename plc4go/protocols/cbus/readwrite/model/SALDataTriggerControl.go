...
func (m *_SALDataTriggerControlBuilder) WithTriggerControlDataBuilder(builderSupplier func(TriggerControlDataBuilder) TriggerControlDataBuilder) SALDataTriggerControlBuilder {
	builder := builderSupplier(m.TriggerControlData.CreateTriggerControlDataBuilder())
	var err error
	m.TriggerControlData, err = builder.Build()
	if err != nil {
		if m.err == nil {
			m.err = &utils.MultiError{MainError: errors.New("sub builder failed")}
		}
		m.err.Append(errors.Wrap(err, "TriggerControlDataBuilder failed"))
	}
	return m
}
