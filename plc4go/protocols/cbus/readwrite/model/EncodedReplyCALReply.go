...
func (m *_EncodedReplyCALReplyBuilder) WithCalReplyBuilder(builderSupplier func(CALReplyBuilder) CALReplyBuilder) EncodedReplyCALReplyBuilder {
	builder := builderSupplier(m.CalReply.CreateCALReplyBuilder())
	var err error
	m.CalReply, err = builder.Build()
	if err != nil {
		if m.err == nil {
			m.err = &utils.MultiError{MainError: errors.New("sub builder failed")}
		}
		m.err.Append(errors.Wrap(err, "CALReplyBuilder failed"))
	}
	return m
}
