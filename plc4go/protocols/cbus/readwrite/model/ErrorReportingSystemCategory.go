...
func (m *_ErrorReportingSystemCategoryBuilder) WithSystemCategoryTypeBuilder(builderSupplier func(ErrorReportingSystemCategoryTypeBuilder) ErrorReportingSystemCategoryTypeBuilder) ErrorReportingSystemCategoryBuilder {
	builder := builderSupplier(m.SystemCategoryType.CreateErrorReportingSystemCategoryTypeBuilder())
	var err error
	m.SystemCategoryType, err = builder.Build()
	if err != nil {
		if m.err == nil {
			m.err = &utils.MultiError{MainError: errors.New("sub builder failed")}
		}
		m.err.Append(errors.Wrap(err, "ErrorReportingSystemCategoryTypeBuilder failed"))
	}
	return m
}
