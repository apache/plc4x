...
func (m UnitStatus) GetLengthInBits(ctx context.Context) uint64 {
	return 8
}

func (m UnitStatus) GetLengthInBytes(ctx context.Context) uint64 {
...
func UnitStatusParseWithBuffer(ctx context.Context, readBuffer utils.ReadBuffer) (UnitStatus, error) {
	log := zerolog.Ctx(ctx)
	_ = log
	val, err := /*TODO: migrate me*/ /*TODO: migrate me*/ readBuffer.ReadUint8("UnitStatus", 8)
	if err != nil {
		return 0, errors.Wrap(err, "error reading UnitStatus")
	}
	if enum, ok := UnitStatusByValue(val); !ok {
		log.Debug().Interface("val", val).Msg("no value val found for UnitStatus")
...
func (e UnitStatus) SerializeWithWriteBuffer(ctx context.Context, writeBuffer utils.WriteBuffer) error {
	log := zerolog.Ctx(ctx)
	_ = log
	return /*TODO: migrate me*/ writeBuffer.WriteUint8("UnitStatus", 8, uint8(uint8(e)), utils.WithAdditionalStringRepresentation(e.PLC4XEnumName()))
}
