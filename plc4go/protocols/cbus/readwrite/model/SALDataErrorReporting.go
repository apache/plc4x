...
func (m *_SALDataErrorReportingBuilder) WithErrorReportingDataBuilder(builderSupplier func(ErrorReportingDataBuilder) ErrorReportingDataBuilder) SALDataErrorReportingBuilder {
	builder := builderSupplier(m.ErrorReportingData.CreateErrorReportingDataBuilder())
	var err error
	m.ErrorReportingData, err = builder.Build()
	if err != nil {
		if m.err == nil {
			m.err = &utils.MultiError{MainError: errors.New("sub builder failed")}
		}
		m.err.Append(errors.Wrap(err, "ErrorReportingDataBuilder failed"))
	}
	return m
}
