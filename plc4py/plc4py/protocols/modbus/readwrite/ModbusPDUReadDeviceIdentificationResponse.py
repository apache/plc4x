...
class ModbusPDUReadDeviceIdentificationResponse(ModbusPDU):
    ...
    def serialize_modbus_pdu_child(self, write_buffer: WriteBuffer):
        ...
        write_buffer.write_bit(self.individual_access,bit_length=1,logical_name="individualAccess")
        ...
        write_buffer.write_unsigned_byte(self.next_object_id,bit_length=8,logical_name="nextObjectId")
        ...
        write_buffer.write_unsigned_byte(number_of_objects, logical_name="number_of_objects")
    ...
    def static_parse_builder(read_buffer: ReadBuffer, response: bool):
        ...
        MEI_TYPE: int = read_buffer.read_unsigned_byte(logical_name="mei_type", response=response)

        level: ModbusDeviceInformationLevel = read_buffer.read_enum(read_function=ModbusDeviceInformationLevel,bit_length=8, logical_name="level", response=response)



        individual_access: bool = read_buffer.read_bit(logical_name="individual_access", bit_length=1, response=response)  



        conformity_level: ModbusDeviceInformationConformityLevel = read_buffer.read_enum(read_function=ModbusDeviceInformationConformityLevel,bit_length=7, logical_name="conformity_level", response=response)



        more_follows: ModbusDeviceInformationMoreFollows = read_buffer.read_enum(read_function=ModbusDeviceInformationMoreFollows,bit_length=8, logical_name="more_follows", response=response)



        next_object_id: int = read_buffer.read_unsigned_byte(logical_name="next_object_id", bit_length=8, response=response)  



        number_of_objects: int = read_buffer.read_unsigned_byte(logical_name="number_of_objects", response=response)

        objects: List[Any] = read_buffer.read_array_field(logical_name="objects", read_function=ModbusDeviceInformationObject.static_parse, count=number_of_objects, response=response)
...
class ModbusPDUReadDeviceIdentificationResponseBuilder:
    ...
    def build(self,) -> ModbusPDUReadDeviceIdentificationResponse:
        modbus_pduread_device_identification_response: ModbusPDUReadDeviceIdentificationResponse = ModbusPDUReadDeviceIdentificationResponse(self.level, self.individual_access, self.conformity_level, self.more_follows, self.next_object_id, self.objects )
