...
class UmasPDUWriteVariableRequest(UmasPDUItem):
    ...
    def serialize_umas_pduitem_child(self, write_buffer: WriteBuffer):
        ...
        write_buffer.write_unsigned_byte(self.variable_count,bit_length=8,logical_name="variableCount")
    ...
    def static_parse_builder(read_buffer: ReadBuffer, umas_request_function_key: int, byte_length: int):
        ...
        crc: int = read_buffer.read_unsigned_int(logical_name="crc", bit_length=32, byte_order=ByteOrder.LITTLE_ENDIAN, umas_request_function_key=umas_request_function_key, byte_length=byte_length)  



        variable_count: int = read_buffer.read_unsigned_byte(logical_name="variable_count", bit_length=8, byte_order=ByteOrder.LITTLE_ENDIAN, umas_request_function_key=umas_request_function_key, byte_length=byte_length)  



        variables: List[Any] = read_buffer.read_array_field(logical_name="variables", read_function=VariableWriteRequestReference.static_parse, count=variable_count, byte_order=ByteOrder.LITTLE_ENDIAN, umas_request_function_key=umas_request_function_key, byte_length=byte_length)
    ...
    def equals(self, o: object) -> bool:
        ...
        return (self.crc == that.crc) and (self.variable_count == that.variable_count) and (self.variables == that.variables) and super().equals(that) and True
...
class UmasPDUWriteVariableRequestBuilder:
    ...
    def build(self,byte_length: int , pairing_key ) -> UmasPDUWriteVariableRequest:
        umas_pduwrite_variable_request: UmasPDUWriteVariableRequest = UmasPDUWriteVariableRequest(byte_length , pairing_key , self.crc, self.variable_count, self.variables )
        return umas_pduwrite_variable_request
