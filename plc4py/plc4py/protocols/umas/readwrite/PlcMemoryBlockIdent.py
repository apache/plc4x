...
class PlcMemoryBlockIdent():
    ...
    def serialize(self, write_buffer: WriteBuffer):
        ...
        write_buffer.write_unsigned_byte(self.block_type,bit_length=8,logical_name="blockType")
    ...
    def static_parse_context(read_buffer: ReadBuffer):
        ...
        block_type: int = read_buffer.read_unsigned_byte(logical_name="block_type", bit_length=8)  
        ...
        status: int = read_buffer.read_unsigned_short(logical_name="status", bit_length=16)  



        memory_length: int = read_buffer.read_unsigned_int(logical_name="memory_length", bit_length=32)  



        read_buffer.pop_context("PlcMemoryBlockIdent")
        # Create the instance
        _plc_memory_block_ident: PlcMemoryBlockIdent = PlcMemoryBlockIdent(block_type, folio, status, memory_length )
    ...
    def equals(self, o: object) -> bool:
        ...
        return (self.block_type == that.block_type) and (self.folio == that.folio) and (self.status == that.status) and (self.memory_length == that.memory_length) and True
