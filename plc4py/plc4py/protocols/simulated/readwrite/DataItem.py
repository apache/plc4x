...
class DataItem:
    ...
    @staticmethod
    def static_serialize(write_buffer: WriteBuffer, _value: PlcValue, data_type: str, number_of_values: int, byte_order: ByteOrder) -> None:
    ...
    @staticmethod
    def get_length_in_bytes(_value: PlcValue, data_type: str, number_of_values: int) -> int:
        return int(math.ceil(float(DataItem.get_length_in_bits(_value, data_type, number_of_values)) / 8.0))


    @staticmethod
    def get_length_in_bits(_value: PlcValue, data_type: str, number_of_values: int) -> int:
        size_in_bits: int = 0
        if  data_type == "BOOL" and number_of_values == int(1): # BOOL
                # Simple Field (value)
                size_in_bits += 1
