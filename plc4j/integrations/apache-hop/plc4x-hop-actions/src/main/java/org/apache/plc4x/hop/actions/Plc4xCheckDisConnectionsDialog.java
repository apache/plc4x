...
public class Plc4xCheckDisConnectionsDialog extends ActionDialog implements IActionDialog {
  private static final Class<?> PKG = Plc4xCheckDisConnectionsDialog.class; // Needed by Translator

  private Shell shell;

  private Text wName;

  private Plc4xCheckDisConnections action;
  ...
  public Plc4xCheckDisConnectionsDialog(Shell parent, IAction action, WorkflowMeta workflowMeta, IVariables variables) {
    super( parent, workflowMeta, variables );
    this.action = (Plc4xCheckDisConnections) action;
  ...
  public void getConnections() {
          ...
          metadataClass = metadataProvider.getMetadataClassForKey("plc4x-connection");
            ...
            manager.getNames().forEach((s)->{
              try {
                  ...
                  Logger.getLogger(Plc4xCheckDisConnectionsDialog.class.getName()).log(Level.SEVERE, null, ex);
              }
            });
    ...
    for (Plc4xConnection ci : databases) {
      if (ci != null) {
        wFields.add(new String[] {ci.getName(), "0", Plc4xCheckDisConnections.unitTimeDesc[0]});
      }
    }
    wFields.removeEmptyRows();
    wFields.setRowNums();
    ...
    if (action.getConnections() != null) {
      for (int i = 0; i < action.getConnections().length; i++) {
        ...
        if (action.getConnections()[i] != null) {
          ...
          ti.setText(3, Plc4xCheckDisConnections.getWaitTimeDesc(action.getWaittimes()[i]));
        }
      }
      wFields.setRowNums();
      wFields.optWidth(true);
    ...
    for (int i = 0; i < nrItems; i++) {
        ...
        try {
            ...
            if (conn != null) {
                ...
                waittimes[i] =
                    Plc4xCheckDisConnections.getWaitTimeByDesc(wFields.getNonEmpty(i).getText(3));
          }            
        } catch (HopException ex) {
            Logger.getLogger(Plc4xCheckDisConnectionsDialog.class.getName()).log(Level.SEVERE, null, ex);
        }
