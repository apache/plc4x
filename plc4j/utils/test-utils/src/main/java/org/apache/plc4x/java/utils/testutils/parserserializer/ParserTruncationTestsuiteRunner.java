...
/**
 * Parses every prefix of every reference message of a testsuite, requiring each one to fail, if it
 * fails, with a {@link BufferException} rather than with an unchecked exception. See
 * {@link #getTruncationTests()}.
 * <p>
 * This is separate from {@link ParserSerializerTestsuiteRunner} so that a protocol whose reference
 * xml has drifted can still be held to it: the two answer different questions, and the corpus is
 * useful for this one whether or not it still describes what the parser produces.
 */
public class ParserTruncationTestsuiteRunner extends XmlTestsuiteLoader {

    private static final Logger LOGGER = LoggerFactory.getLogger(ParserTruncationTestsuiteRunner.class);

    protected final Set<String> ignoredTestCases = new HashSet<>();

    public ParserTruncationTestsuiteRunner(String testsuiteDocument, String... ignoredTestCases) {
    ...
    protected ParserSerializerTestsuite parseTestsuite() throws ParserSerializerTestsuiteException {
        try {
