...
import org.apache.plc4x.java.utils.pcapsockets.PcapSocketException;
 ...
 */
public class PcapSocketChannel extends OioByteStreamChannel {

    private static final Logger logger = LoggerFactory.getLogger(PcapSocketChannel.class);

    private final PcapSocketChannelConfig config;

    private PcapSocketAddress remoteRawSocketAddress;
    private SocketAddress localAddress;
    private PcapHandle handle;
    private Thread loopThread;

    public PcapSocketChannel() {
        super(null);
        config = new PcapSocketChannelConfig(this);
    }
    ...
    protected void doConnect(SocketAddress remoteAddress, SocketAddress localAddress) throws Exception {
        if(!(remoteAddress instanceof PcapSocketAddress)) {
            logger.error("Expecting remote address of type PcapSocketAddress");
            pipeline().fireExceptionCaught(
                new PcapSocketException("Expecting remote address of type PcapSocketAddress"));
            return;
        }
        ...
        if(!pcapFile.exists()) {
            ...
            pipeline().fireExceptionCaught(new PcapSocketException(String.format(
