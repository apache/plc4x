// Licensed to the Apache Software Foundation (ASF) under one
// or more contributor license agreements.  See the NOTICE file
// distributed with this work for additional information
// regarding copyright ownership.  The ASF licenses this file
// to you under the Apache License, Version 2.0 (the
// "License"); you may not use this file except in compliance
// with the License.  You may obtain a copy of the License at
//
//   https://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing,
// software distributed under the License is distributed on an
// "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
// KIND, either express or implied.  See the License for the
// specific language governing permissions and limitations
// under the License.
package org.apache.plc4x.java.cbus.readwrite;

import org.apache.plc4x.java.spi.buffers.api.Message;
import org.apache.plc4x.java.spi.buffers.api.ReadBuffer;
import org.apache.plc4x.java.spi.buffers.api.WithOption;
import org.apache.plc4x.java.spi.buffers.api.WriteBuffer;
import org.apache.plc4x.java.spi.buffers.api.exceptions.BufferException;
import org.apache.plc4x.java.spi.fields.data.reader.DataReaderFactory;
import org.apache.plc4x.java.spi.fields.data.writer.DataWriterFactory;
import org.apache.plc4x.java.spi.fields.exceptions.ParseAssertException;
import org.apache.plc4x.java.spi.fields.fields.reader.FieldReaderFactory;
import org.apache.plc4x.java.spi.fields.fields.writer.FieldWriterFactory;
import org.apache.plc4x.java.spi.fields.utils.ThreadLocalHelper;

/**
 * Code generated by code-generation. DO NOT EDIT.
 */
public class ParameterValueApplicationAddress2 extends ParameterValue implements Message {
  protected final ApplicationAddress2 value;

  protected final byte[] data;

  public ParameterValueApplicationAddress2(ApplicationAddress2 value, byte[] data) {
    this.value = value;
    this.data = data;
  }

  /**
   * Discriminator field parameterType
   */
  @Override
  public ParameterType getParameterType() {
    return ParameterType.APPLICATION_ADDRESS_2;
  }

  /**
   * Property field value
   */
  public ApplicationAddress2 getValue() {
    return value;
  }

  /**
   * Property field data
   */
  public byte[] getData() {
    return data;
  }

  public static ParameterValueBuilder staticParseParameterValueBuilder(ReadBuffer readBuffer,
      ParameterType parameterType, short numBytes) throws BufferException {
    readBuffer.pushContext(WithOption.WithName("ParameterValueApplicationAddress2"));
    int startPos = readBuffer.getPositionInBits();
    boolean _lastItem = ThreadLocalHelper.lastItemThreadLocal.get();
    // Validation Field
    if(!((numBytes) >= (1))) {
      throw new ParseAssertException("ApplicationAddress2 has exactly one byte");
    }

    // Simple Field: value
    ApplicationAddress2 value = FieldReaderFactory.readSimpleField(DataReaderFactory.readComplex(() -> (ApplicationAddress2) ApplicationAddress2.staticParse(readBuffer), readBuffer), WithOption.WithName("value"));

    // Array Field: data
    byte[] data = readBuffer.readBits(Math.toIntExact(((numBytes) - (1)) * 8), WithOption.WithName("data"));

    readBuffer.popContext();
    return new ParameterValueBuilderImpl(value, data);
  }

  protected void serializeParameterValueChild(WriteBuffer writeBuffer) throws BufferException {
    writeBuffer.pushContext(WithOption.WithName("ParameterValueApplicationAddress2"));
    int startPos = writeBuffer.getPositionInBits();
    boolean _lastItem = ThreadLocalHelper.lastItemThreadLocal.get();
    // Validation Field (Nothing needed here)

    // Simple Field: value
    FieldWriterFactory.writeSimpleField((ApplicationAddress2) value, DataWriterFactory.writeComplex(writeBuffer), WithOption.WithName("value"));

    // Array Field: data
    FieldWriterFactory.writeByteArrayField(data, DataWriterFactory.writeByteArray(writeBuffer, (int) ((data != null) ? data.length : 0)), WithOption.WithName("data"));

    writeBuffer.popContext();
  }

  @Override
  public int getLengthInBytes() {
    return (int) Math.ceil((float) getLengthInBits() / 8.0);
  }

  @Override
  public int getLengthInBits() {
    int lengthInBits = super.getLengthInBits();
    ParameterValueApplicationAddress2 _value = this;
    boolean _lastItem = ThreadLocalHelper.lastItemThreadLocal.get();
    // Validation Field (Nothing needed here)

    // Simple Field: value
    lengthInBits += value.getLengthInBits();

    // Array Field: data
    lengthInBits += 8 * ((data != null) ? data.length : 0);

    return lengthInBits;
  }

  public static class ParameterValueBuilderImpl implements ParameterValue.ParameterValueBuilder {
    private final ApplicationAddress2 value;

    private final byte[] data;

    public ParameterValueBuilderImpl(ApplicationAddress2 value, byte[] data) {
      this.value = value;
      this.data = data;
    }

    public ParameterValue build() {
      return new ParameterValueApplicationAddress2(value, data);
    }
  }
}
