...
public class FirmataTagHandler implements PlcTagHandler {
    ...
    public PlcTag parseTag(String tagAddress) {
        if (FirmataTagDigital.ADDRESS_PATTERN.matcher(tagAddress).matches()) {
            return FirmataTagDigital.of(tagAddress);
        } else if (FirmataTagAnalog.ADDRESS_PATTERN.matcher(tagAddress).matches()) {
            return FirmataTagAnalog.of(tagAddress);
        }
