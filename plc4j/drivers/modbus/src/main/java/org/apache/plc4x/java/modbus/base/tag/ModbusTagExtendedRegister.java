...
public class ModbusTagExtendedRegister extends ModbusTag {
    ...
    public ModbusTagExtendedRegister(int address, Integer quantity, ModbusDataType dataType, Map<String, String> config) {
        super(address, quantity, dataType, config);
    }

    public ModbusTagExtendedRegister(int address, Integer quantity, Integer stringLength, ModbusDataType dataType, Map<String, String> config) {
    ...
    public static ModbusTagExtendedRegister of(String addressString) {
        ...
        if ((address + registers) > REGISTER_MAXADDRESS) {
            throw new IllegalArgumentException("Last requested address is out of range, should be between 0 and " + REGISTER_MAXADDRESS + ". Was " + (address + (registers - 1)));
        }
