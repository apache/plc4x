 ...
 */
class ModbusArrayParityTest {

    @Test
    void aRangeOfEightStartingAtZero() {
        List<ArrayInfo> dimensions = ModbusTag.of("holding-register:1[0..7]:INT").getArrayInfo();
    ...
    void aBareIndexIsAScalar() {
        assertTrue(ModbusTag.of("holding-register:1[4]:INT").getArrayInfo().isEmpty());
    }

    @Test
    void anOmittedSelectionIsAScalar() {
        assertTrue(ModbusTag.of("holding-register:1:INT").getArrayInfo().isEmpty());
    }
}
