...
/**
 * Manual smoke-test for the Modbus/TCP driver's <em>emulated</em> subscription path.
 * <p>
 * Modbus has no native subscription mechanism; since {@code ModbusTcpConnection} now extends
 * {@code PollingSubscriptionConnectionBase}, subscriptions are emulated by polling the tag via the
 * regular read path. This test registers two subscriptions against {@code holding-register:3:WORD}:
 * <ul>
 ...
 * <p>
 * To produce visible value changes, write to holding register 3 on the PLC while this is running.
 */
