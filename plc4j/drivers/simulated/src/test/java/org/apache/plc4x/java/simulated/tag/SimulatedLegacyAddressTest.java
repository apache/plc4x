 ...
 */
class SimulatedLegacyAddressTest {

    @Test
    void legacyForm0IsRejected() {
        assertThrows(PlcInvalidTagException.class, () -> SimulatedTag.of("RANDOM/foo:INT[4]"));
    }

    @Test
    void theReplacementFormParses() {
        assertNotNull(SimulatedTag.of("RANDOM/foo[0..3]:INT"));
    }
    ...
    void theErrorNamesTheReplacementAddress() {
        PlcInvalidTagException thrown =
            assertThrows(PlcInvalidTagException.class, () -> SimulatedTag.of("RANDOM/foo:INT[4]"));
        assertTrue(thrown.getMessage().contains("RANDOM/foo[0..3]:INT"), thrown::getMessage);
    }
}
