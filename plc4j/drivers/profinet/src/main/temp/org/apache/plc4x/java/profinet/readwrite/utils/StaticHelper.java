...
public class StaticHelper {
    ...
    public static short calculateUdpChecksum(IpAddress sourceAddress, IpAddress destinationAddress, int sourcePort, int destPort, int packetLength, DceRpc_Packet payload) {
        ...
        if (evenSize) {
            buffer = new WriteBufferByteBased(new byte[payload.getLengthInBytes()], ByteOrderBigEndian.optionByteOrderBigEndian());
        } else {
            buffer = new WriteBufferByteBased(new byte[payload.getLengthInBytes() + 1], ByteOrderBigEndian.optionByteOrderBigEndian());
        }

        try {
            payload.serialize(buffer);
            if (!evenSize) {
                buffer.writeSignedByte(8, (byte) 0x00, WithOption.WithName("Padding"));
            }
    ...
    public static boolean isSysexEnd(ReadBuffer io) {
        ...
        try {
            return io.readSignedByte(8) == (byte) 0xF7;
        } catch (BufferException e) {
            return false;
        } finally {
    ...
    public static PnIo_CyclicServiceDataUnit readDataUnit(ReadBuffer readBuffer) throws BufferException {
        ...
        int initialPos = readBuffer.getPositionInBits();
        while (readBuffer.getRemainingBits() >= 8) {
            readBuffer.readSignedByte(8);
        }
        int dataUnitLength = readBuffer.getPositionInBits() - initialPos - NO_TRAILING_BYTES;
        readBuffer.setPositionInBits(initialPos);
