 ...
 */
class ProfinetLegacyAddressTest {

    @Test
    void legacyForm0IsRejected() {
        assertThrows(PlcInvalidTagException.class, () -> ProfinetTag.of("foo.bar:DINT[4]"));
    }

    @Test
    void theReplacementFormParses() {
        assertNotNull(ProfinetTag.of("foo.bar[0..3]:DINT"));
    }
    ...
    void theErrorNamesTheReplacementAddress() {
        PlcInvalidTagException thrown =
            assertThrows(PlcInvalidTagException.class, () -> ProfinetTag.of("foo.bar:DINT[4]"));
        assertTrue(thrown.getMessage().contains("foo.bar[0..3]:DINT"), thrown::getMessage);
    }
}
