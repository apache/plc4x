 ...
 */
class EipArrayParityTest {

    @Test
    void aRangeOfEightStartingAtZero() {
        List<ArrayInfo> dimensions = EipTag.of("myArray[0..7]:DINT").getArrayInfo();
    ...
    void aBareIndexIsAScalar() {
        assertTrue(EipTag.of("myArray[4]:DINT").getArrayInfo().isEmpty());
    }

    @Test
    void anOmittedSelectionIsAScalar() {
        assertTrue(EipTag.of("myArray:DINT").getArrayInfo().isEmpty());
    }
}
