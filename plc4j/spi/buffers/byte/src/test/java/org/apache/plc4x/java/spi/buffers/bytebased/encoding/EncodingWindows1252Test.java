...
class EncodingWindows1252Test extends BaseEncodingDefaultTest {
    private EncodingWindows1252 encoding;

    @BeforeEach
    void setUp() {
        encoding = new EncodingWindows1252();
    }

    @Test
    void getName() {
        assertEquals("WINDOWS1252", encoding.getName());
    }

    @Test
    void optionFactory() {
        WithOption withOption = EncodingWindows1252.optionEncodingWINDOWS1252();
        Optional<String> stringEncoding = WithOption.extractEncoding(new WithOption[]{withOption});
        assertTrue(stringEncoding.isPresent());
        assertEquals("WINDOWS1252", stringEncoding.get());
    }
    ...
    void encodeAndDecodeStringWithSpecialCharacters() {
        assertEncodeDecode("Café", 32, wrapDecoder(encoding::decodeString), wrapEncoder(encoding::encodeString));
        assertEncodeDecode("Grüße", 40, wrapDecoder(encoding::decodeString), wrapEncoder(encoding::encodeString));
        assertEncodeDecode("Straße", 48, wrapDecoder(encoding::decodeString), wrapEncoder(encoding::encodeString));
        assertEncodeDecode("naïve", 40, wrapDecoder(encoding::decodeString), wrapEncoder(encoding::encodeString));
    }
    ...
    void encodeAndDecodeShortHappyPath() {
        assertEncodeDecode((short) 12345, 40, wrapDecoder(encoding::decodeShort), wrapEncoder(encoding::encodeShort));
    ...
    void encodeAndDecodeIntHappyPath() {
        assertEncodeDecode(1234567890, 80, wrapDecoder(encoding::decodeInt), wrapEncoder(encoding::encodeInt));
    ...
    void encodeAndDecodeLongHappyPath() {
        assertEncodeDecode(1234567890123456789L, 152, wrapDecoder(encoding::decodeLong), wrapEncoder(encoding::encodeLong));
    ...
    void encodeAndDecodeFloatHappyPath() {
        assertEncodeDecode(123.456f, 56, wrapDecoder(encoding::decodeFloat), wrapEncoder(encoding::encodeFloat), 0.0001f);
    ...
    void encodeAndDecodeDoubleHappyPath() {
        assertEncodeDecode(123.456789, 80, wrapDecoder(encoding::decodeDouble), wrapEncoder(encoding::encodeDouble), 0.0000001);
    ...
    @Test
    void encodeAndDecodeWindows1252SpecificCharacters() {
        // Test characters specific to Windows-1252 encoding
        assertEncodeDecode("€", 8, wrapDecoder(encoding::decodeString), wrapEncoder(encoding::encodeString)); // Euro symbol
        assertEncodeDecode("™", 8, wrapDecoder(encoding::decodeString), wrapEncoder(encoding::encodeString)); // Trademark
        assertEncodeDecode("©", 8, wrapDecoder(encoding::decodeString), wrapEncoder(encoding::encodeString)); // Copyright
        assertEncodeDecode("®", 8, wrapDecoder(encoding::decodeString), wrapEncoder(encoding::encodeString)); // Registered
    }
    ...
    @Test
    void encodeStringWithUnsupportedCharacters() {
        // Test characters not supported by Windows-1252 (should be replaced)
        String input = "Hello 世界!"; // Chinese characters not in Windows-1252
