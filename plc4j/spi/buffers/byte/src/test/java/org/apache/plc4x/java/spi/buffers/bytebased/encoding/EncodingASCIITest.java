...
class EncodingASCIITest extends BaseEncodingDefaultTest {

    private EncodingASCII encoding;

    @BeforeEach
    void setUp() {
        encoding = new EncodingASCII();
    }

    @Test
    void getName() {
        assertEquals("ASCII", encoding.getName());
    }

    @Test
    void optionFactory() {
        WithOption withOption = EncodingASCII.optionEncodingASCII();
        Optional<String> stringEncoding = WithOption.extractEncoding(new WithOption[]{withOption});
        assertTrue(stringEncoding.isPresent());
        assertEquals("ASCII", stringEncoding.get());
    }
