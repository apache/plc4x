...
class EncodingUTF16LETest extends BaseEncodingDefaultTest {
    private EncodingUTF16LE encoding;

    @BeforeEach
    void setUp() {
        encoding = new EncodingUTF16LE();
    }

    @Test
    void getName() {
        assertEquals("UTF16LE", encoding.getName());
    }

    @Test
    void optionFactory() {
        WithOption withOption = EncodingUTF16LE.optionEncodingUTF16LE();
        Optional<String> stringEncoding = WithOption.extractEncoding(new WithOption[]{withOption});
        assertTrue(stringEncoding.isPresent());
        assertEquals("UTF16LE", stringEncoding.get());
    }
    ...
    void decodeStringWithInsufficientBytes() {
        byte[] encoded = new byte[]{65, 0, 0}; // Incomplete UTF-16LE sequence
        int numBits = 32;
    ...
    void testByteOrderMark() throws BufferException {
        // UTF-16LE should not include BOM
        String input = "A";
        byte[] encoded = encoding.encodeString(16, input);
        assertEquals(2, encoded.length);
        assertEquals(65, encoded[0]); // Low byte ('A')
        assertEquals(0, encoded[1]); // High byte
    }
    ...
    void testSurrogatePairEncoding() throws BufferException {
        ...
        // Should be encoded as surrogate pair D834 DD1E in little-endian
        assertEquals((byte) 0x34, encoded[0]);
        assertEquals((byte) 0xD8, encoded[1]);
        assertEquals((byte) 0x1E, encoded[2]);
        assertEquals((byte) 0xDD, encoded[3]);
    }
}