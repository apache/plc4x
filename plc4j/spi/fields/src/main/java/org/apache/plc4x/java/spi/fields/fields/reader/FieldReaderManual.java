...
public class FieldReaderManual<T> implements FieldCommons {
    ...
    public T readManualField(ReadBuffer readBuffer, ParseSupplier<T> parseFunction, WithOption... options) throws BufferException {
        LOGGER.debug("reading field {}", getName(options));
        readBuffer.pushContext(options);
        T value = parseFunction.get();// switchParseByteOrderIfNecessary(parseFunction::get, readBuffer, extractByteOrder(readerArgs).orElse(null));
