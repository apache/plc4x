...
public class FieldWriterPadding<T> implements FieldCommons {
    ...
    public void writePaddingField(int timesPadding, T value, DataWriter<T> dataWriter, WithOption... options) throws BufferException {
        LOGGER.debug("write field {}", getName(options));
        dataWriter.pushContext(WithOption.AddOptions(options, WithOption.WithRenderAsList(true), WithOption.WithName("padding")));
        for (int i = 0; i < timesPadding; i++) {
            dataWriter.write(value, options);
        }
        dataWriter.popContext(WithOption.AddOptions(options, WithOption.WithRenderAsList(true), WithOption.WithName("padding")));
    }
