...
public class FieldReaderSimple<T> implements FieldCommons {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(FieldReaderSimple.class);

    public T readSimpleField(DataReader<T> dataReader, WithOption... options) throws BufferException {
        LOGGER.debug("reading field {}", getName(options));
        return dataReader.read(options);
    }
