...
public class FieldWriterManual<T> implements FieldCommons {
    ...
    public void writeManualField(RunSerializeWrapped consumer, WriteBuffer writeBuffer, WithOption... options) throws BufferException {
        LOGGER.debug("write field {}", getName(options));
        consumer.run();
        //switchSerializeByteOrderIfNecessary(consumer, writeBuffer, extractByteOrder(options).orElse(null));
    }
