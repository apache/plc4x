...
public class FieldReaderUnknown<T> implements FieldCommons {

    private static final Logger LOGGER = LoggerFactory.getLogger(FieldReaderUnknown.class);

    public T readUnknownField(DataReader<T> dataReader, WithOption... options) throws BufferException {
        LOGGER.debug("reading field {}", getName(options));
        T unknownValue = dataReader.read(options);
