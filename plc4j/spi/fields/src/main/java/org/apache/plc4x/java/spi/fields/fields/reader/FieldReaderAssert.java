...
public class FieldReaderAssert<T> implements FieldCommons {
    ...
    public T readAssertField(DataReader<T> dataReader, T expectedValue, WithOption... options) throws BufferException {
        LOGGER.debug("reading field {}", getName(options));
        T assertValue = dataReader.read(options);
        if (!Objects.equals(assertValue, expectedValue)) {
            throw new ParseAssertException("Field: " + getName(options) + ": Actual value " + assertValue + " doesn't match expected " + expectedValue);
        }
        return assertValue;
    }
}
