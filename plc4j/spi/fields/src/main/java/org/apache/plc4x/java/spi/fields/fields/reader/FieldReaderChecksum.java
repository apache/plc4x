...
public class FieldReaderChecksum<T> implements FieldCommons {
    ...
    public T readChecksumField(DataReader<T> dataReader, T referenceValue, WithOption... options) throws BufferException {
        LOGGER.debug("reading field {}", getName(options));
        T checksumValue = dataReader.read(options);
        if (!Objects.equals(checksumValue, referenceValue)) {
            throw new BufferException("Checksum value '" + checksumValue + "' doesn't match expected '" + referenceValue + "'");
        }
