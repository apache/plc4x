...
class FieldReaderOptionalTest {

    @Test
    void conditionFalseReturnsNull() throws Exception {
        FieldReaderOptional<Integer> reader = new FieldReaderOptional<>();
        SimpleDataReader<Integer> dr = new SimpleDataReader<>(() -> 99);
        assertNull(reader.readOptionalField(dr, false));
    }

    @Test
    void conditionTrueReadsValue() throws Exception {
        FieldReaderOptional<Integer> reader = new FieldReaderOptional<>();
        SimpleDataReader<Integer> dr = new SimpleDataReader<>(() -> 99);
        assertEquals(99, reader.readOptionalField(dr, true));
    }

    @Test
    void parseAssertExceptionReturnsNullAndRestoresPosition() throws Exception {
        FieldReaderOptional<Integer> reader = new FieldReaderOptional<>();
        // Custom DataReader that throws ParseAssertException and moves position before
        DataReader<Integer> dr = new DataReader<>() {
            final TestBuffers.DummyReadBuffer rb = new TestBuffers.DummyReadBuffer();
            @Override public Integer read(WithOption... options) throws BufferException { rb.setPositionInBits(123); throw new ParseAssertException("fail"); }
        ...
        int posBefore = dr.getPositionInBits();
        assertNull(reader.readOptionalField(dr, true));
        assertEquals(posBefore, dr.getPositionInBits());
    }
}
