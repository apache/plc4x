...
public class ConfigurationFactory {
    ...
    public <T extends Configuration> T createConfiguration(Class<T> pClazz, String paramString) {
        // Get a map of all parameters in the connection string.
        Map<String, List<String>> paramStringValues = splitQuery(paramString);

        return createConfiguration(pClazz, paramStringValues);
    }

    public <T extends Configuration> T createPrefixedConfiguration(Class<T> pClazz, String prefix, String paramString) {
        // Get a map of all parameters in the connection string.
        Map<String, List<String>> paramStringValues = splitQuery(paramString);
        ...
        return createConfiguration(pClazz, filteredParamStringValues);
    }

    public <T> T createConfiguration(Class<T> pClazz, Map<String, List<String>> paramStringValues) {
        ...
        Map<String, Field> fields = Arrays.stream(FieldUtils.getAllFields(pClazz))
            ...
            // - Create a map with the field-name as a key and the field itself as the value.
        ...
        try {
            ...
            for (Map.Entry<String, Field> entry : fields.entrySet()) {
                ...
                if (field.getAnnotation(ComplexConfigurationParameter.class) != null) {
                    ...
                    // TODO: Ensure the field type is of the right type.
                    Class<Configuration> configType = (Class<Configuration>) field.getType();
                    Configuration configValue = createConfiguration(configType, filteredParamStringValues);
    ...
    public static <T> T configure(Configuration configuration, T obj) {
        ...
        if (ClassUtils.isAssignable(obj.getClass(), HasConfiguration.class)) {
            ...
            if (typeOptional.isPresent()) {
                ...
                if (configType instanceof Class<?> configClass) {
    ...
    public static Object getDefaultValueFromAnnotation(Field field) {
        ShortDefaultValue shortDefaultValue = field.getAnnotation(ShortDefaultValue.class);
        if (shortDefaultValue != null) {
            return shortDefaultValue.value();
        }
