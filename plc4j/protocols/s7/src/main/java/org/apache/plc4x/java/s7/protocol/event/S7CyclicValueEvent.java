...
import static org.apache.plc4x.java.s7.utils.S7Helper.decodeResponseCode;

/**
 *
 * @author cgarcia
 */
public class S7CyclicValueEvent implements PlcSubscriptionEvent {

    private final Map<String, AssociatedValueItem> items;
    private final Instant instant;
    
    public S7CyclicValueEvent(Map<String, AssociatedValueItem> items) {
        this.items = items;
    ...
    public Byte[] getByteArray(String name) {
        byte[] bytes = new byte[items.get(name).getData().readableBytes()];
        items.get(name).getData().duplicate().readBytes(bytes);
    ...
    public PlcResponseCode getResponseCode(String name) {
        return decodeResponseCode(items.get(name).getReturnCode());
    }
